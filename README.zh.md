...
brew install anomalyco/tap/kreonyx # macOS 和 Linux（推荐，始终保持最新）
brew install kreonyx              # macOS 和 Linux（官方 brew formula，更新频率较低）
sudo pacman -S kreonyx            # Arch Linux (Stable)
paru -S kreonyx-bin               # Arch Linux (Latest from AUR)
mise use -g kreonyx               # 任意系统
nix run nixpkgs#kreonyx           # 或用 github:anomalyco/kreonyx 获取最新 dev 分支
```
...
KREONYX CLI 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/anomalyco/kreonyx/releases) 或 [opencode.ai/download](https://opencode.ai/download) 下载。
...
KREONYX CLI 内置两种 Agent，可用 `Tab` 键快速切换：
...
如果你在项目名中使用了 “kreonyx”（如 “kreonyx-dashboard” 或 “kreonyx-mobile”），请在 README 里注明该项目不是 KREONYX CLI 团队官方开发，且不存在隶属关系。
...
- 100% 开源。
- 不绑定特定提供商。推荐使用 [KREONYX CLI Zen](https://opencode.ai/zen) 的模型，但也可搭配 Claude、OpenAI、Google 甚至本地模型。模型迭代会缩小差异、降低成本，因此保持 provider-agnostic 很重要。
- 内置 LSP 支持。
- 聚焦终端界面 (TUI)。KREONYX CLI 由 Neovim 爱好者和 [terminal.shop](https://terminal.shop) 的创建者打造，会持续探索终端的极限。
- 客户端/服务器架构。可在本机运行，同时用移动设备远程驱动。TUI 只是众多潜在客户端之一。

---

**加入我们的社区** [Discord](https://discord.gg/kreonyx) | [X.com](https://x.com/kreonyx)
