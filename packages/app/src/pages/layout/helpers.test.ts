...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("kreonyx://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("kreonyx://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("kreonyx://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("kreonyx://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("kreonyx://open-project")).toBeUndefined()
    expect(parseDeepLink("kreonyx://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "kreonyx://open-project?directory=/a",
      "kreonyx://other?directory=/b",
      "kreonyx://open-project?directory=/c",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENCODE__: {
        deepLinks: ["kreonyx://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["kreonyx://open-project?directory=/a"])
