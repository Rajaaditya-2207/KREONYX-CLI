...
export const popularProviders = ["kreonyx", "anthropic", "github-copilot", "openai", "google", "openrouter", "vercel"]
...
export function useProviders() {
  ...
  const paid = createMemo(() =>
    connected().filter((p) => p.id !== "kreonyx" || Object.values(p.models).find((m) => m.cost?.input)),
  )
