...
const kreonyxDir = path.join(repoDir, "packages", "kreonyx")
...
const sandbox = await fs.mkdtemp(path.join(os.tmpdir(), "kreonyx-e2e-"))
...
const serverEnv = {
  ...
  OPENCODE_E2E_MODEL: "kreonyx/gpt-5-nano",
  OPENCODE_CLIENT: "app",
...
try {
  ...
  if (seedExit !== 0) {
    ...
    const log = await import("../../kreonyx/src/util/log")
    const install = await import("../../kreonyx/src/installation")
    await log.Log.init({
      print: true,
    ...
    const servermod = await import("../../kreonyx/src/server/server")
    inst = await import("../../kreonyx/src/project/instance")
    server = servermod.Server.listen({ port: serverPort, hostname: "127.0.0.1" })
    console.log(`kreonyx server listening on http://127.0.0.1:${serverPort}`)
