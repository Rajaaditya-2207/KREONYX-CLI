...
export function Code<T>(props: CodeProps<T>) {
  ...
  const clearHighlightFind = () => {
    ...
    if (!api) return
    api.delete("kreonyx-find")
    api.delete("kreonyx-find-current")
  }
  ...
  const setHighlights = (ranges: Range[], index: number) => {
    ...
    if (active) api.set("kreonyx-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== index)
    if (rest.length > 0) api.set("kreonyx-find", new Highlight(...rest))
    return true
  }
