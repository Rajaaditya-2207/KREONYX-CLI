...
async fn initialize(app: AppHandle) {
    ...
    let sqlite_done = needs_sqlite_migration.then(|| {
        tracing::info!(
            path = %kreonyx_db_path().expect("failed to get db path").display(),
                    ...
                    Some(
                        async move {
                            ...
                            if let Some(err) = err {
                                ...
                                return Err(format!(
                                    "Failed to spawn KREONYX CLI Server ({err}). Logs:\n{}",
                                    get_logs()
                                ));
                            }
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = kreonyx_db_path() else {
        return true;
    };
...
fn kreonyx_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("kreonyx").join("kreonyx.db"))
}
