...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("kreonyx-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("kreonyx_lib=debug,kreonyx_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("kreonyx_lib=info,kreonyx_desktop=info,sidecar=info")
        }
    });
