...
const CLI_INSTALL_DIR: &str = ".kreonyx/bin";
const CLI_BINARY_NAME: &str = "kreonyx";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("kreonyx-install.sh");
...
pub fn spawn_command(
    ...
    let cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.kreonyx/bin/kreonyx\"".to_string(),
...
pub fn serve(
    ...
    let envs = [
        ("OPENCODE_SERVER_USERNAME", "kreonyx".to_string()),
    ...
    .expect("Failed to spawn kreonyx");
