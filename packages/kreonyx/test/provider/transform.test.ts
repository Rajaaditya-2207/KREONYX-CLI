...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, kreonyxModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.kreonyx?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.kreonyx?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              kreonyx: { itemId: "msg_kreonyx_part" },
    ...
    expect(result[0].providerOptions?.kreonyx?.itemId).toBe("msg_kreonyx")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.kreonyx?.itemId).toBe("msg_kreonyx_part")
