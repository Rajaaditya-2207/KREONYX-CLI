...
async function writeManagedSettings(settings: object, filename = "kreonyx.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "kreonyx.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "kreonyx.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "kreonyx.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .kreonyx directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kreonyxDir = path.join(dir, ".kreonyx")
      await fs.mkdir(kreonyxDir, { recursive: true })
      const agentDir = path.join(kreonyxDir, "agent")
...
test("loads agents from .kreonyx/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kreonyxDir = path.join(dir, ".kreonyx")
      await fs.mkdir(kreonyxDir, { recursive: true })

      const agentsDir = path.join(kreonyxDir, "agents")
...
test("loads commands from .kreonyx/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kreonyxDir = path.join(dir, ".kreonyx")
      await fs.mkdir(kreonyxDir, { recursive: true })

      const commandDir = path.join(kreonyxDir, "command")
...
test("loads commands from .kreonyx/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kreonyxDir = path.join(dir, ".kreonyx")
      await fs.mkdir(kreonyxDir, { recursive: true })

      const commandsDir = path.join(kreonyxDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "kreonyx.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .kreonyx config
      const projectDir = path.join(dir, "project")
      const kreonyxDir = path.join(projectDir, ".kreonyx")
      await fs.mkdir(kreonyxDir, { recursive: true })
      ...
      // Local .kreonyx config with different plugins
      await Bun.write(
        path.join(kreonyxDir, "kreonyx.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .kreonyx config with some overlapping plugins
      await Bun.write(
        path.join(kreonyxDir, "kreonyx.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("local .kreonyx config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .kreonyx directory config enables it
      const kreonyxDir = path.join(dir, ".kreonyx")
      await fs.mkdir(kreonyxDir, { recursive: true })
      await Bun.write(
        path.join(kreonyxDir, "kreonyx.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/kreonyx")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/kreonyx")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-kreonyx@2.4.3")).toBe("oh-my-kreonyx")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@kreonyx/plugin@2.0.0")).toBe("@kreonyx/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-kreonyx@2.4.3", "file:///project/.kreonyx/plugin/oh-my-kreonyx.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.kreonyx/plugin/oh-my-kreonyx.js")
  })
  ...
  test("local plugin directory overrides global kreonyx.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const kreonyxDir = path.join(projectDir, ".kreonyx")
        const pluginDir = path.join(kreonyxDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .kreonyx/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENCODE_DISABLE_PROJECT_CONFIG"]
    process.env["OPENCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .kreonyx directory with a command
          const kreonyxDir = path.join(dir, ".kreonyx", "command")
          await fs.mkdir(kreonyxDir, { recursive: true })
          await Bun.write(path.join(kreonyxDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .kreonyx should NOT be in directories list
