...
const AGENT_USERNAME = "kreonyx-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/kreonyx.yml"
      ...
      async fn() {
        {
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/kreonyx-agent"
            const command =
...
jobs:
  kreonyx:
    if: |
      ...
      startsWith(github.event.comment.body, '/kreonyx')
    ...
    steps:
      ...
      - name: Run kreonyx
        uses: anomalyco/kreonyx/github@latest${envStr}
        with:
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      try {
        ...
        console.log("kreonyx session", session.id)
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/kreonyx,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to kreonyx...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("kreonyx-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `kreonyx/${type}-${hex}-${timestamp}`
        }
        return `kreonyx/${type}${issueId}-${timestamp}`
      }
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/kreonyx-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[kreonyx session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the kreonyx infrastructure after your response",
