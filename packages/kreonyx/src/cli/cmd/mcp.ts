    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: kreonyx mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in kreonyx.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .kreonyx/ subdirectory too)
  const candidates = [path.join(baseDir, "kreonyx.json"), path.join(baseDir, "kreonyx.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".kreonyx", "kreonyx.json"), path.join(baseDir, ".kreonyx", "kreonyx.jsonc"))
  }
  ...
  // Default to kreonyx.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., kreonyx x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "kreonyx-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
