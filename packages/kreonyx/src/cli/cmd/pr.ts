...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run kreonyx",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for kreonyx session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found kreonyx session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`kreonyx import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting kreonyx...")
        UI.println()

        // Launch kreonyx TUI with session ID if available
        const { spawn } = await import("child_process")
        const kreonyxArgs = sessionId ? ["-s", sessionId] : []
        const kreonyxProcess = spawn("kreonyx", kreonyxArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          kreonyxProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`kreonyx exited with code ${code}`))
          })
          kreonyxProcess.on("error", reject)
        })
      },
    })
  },
