...
import { EOL } from "os"
import { createOpencodeClient, type Message, type OpencodeClient, type ToolPart } from "@kreonyx/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run kreonyx with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running kreonyx server (e.g., http://localhost:4096)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://kreonyx.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
