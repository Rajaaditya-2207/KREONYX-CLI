...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall kreonyx and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall KREONYX CLI")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g kreonyx",
      pnpm: "pnpm uninstall -g kreonyx",
      bun: "bun remove -g kreonyx",
      yarn: "yarn global remove kreonyx",
      brew: "brew uninstall kreonyx",
      choco: "choco uninstall kreonyx",
      scoop: "scoop uninstall kreonyx",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall kreonyx -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using KREONYX CLI!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# kreonyx") || content.includes(".kreonyx/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".kreonyx/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".kreonyx/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".kreonyx"))
    ) {
      continue
    }
