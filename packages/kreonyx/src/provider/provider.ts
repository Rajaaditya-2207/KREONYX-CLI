...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async kreonyx(input) {
      const hasKey = await (async () => {
        ...
        if (config.provider?.["kreonyx"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from kreonyx.json provider config
    ...
    gitlab: async (input) => {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `kreonyx/${Installation.VERSION} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `kreonyx auth cloudflare-ai-gateway`.",
        )
      }
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "kreonyx",
          },
        },
      }
    },
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("kreonyx")) {
        priority = ["gpt-5-nano"]
      }
    ...
    // Check if kreonyx provider is available before using it
    const kreonyxProvider = await state().then((state) => state.providers["kreonyx"])
    if (kreonyxProvider && kreonyxProvider.models["gpt-5-nano"]) {
      return getModel("kreonyx", "gpt-5-nano")
    }
