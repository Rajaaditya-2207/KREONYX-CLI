...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for kreonyx.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal kreonyx sessions
...
kreonyx acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via kreonyx's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | kreonyx acp
```
...
### Mapping to KREONYX CLI

ACP sessions map cleanly to kreonyx's internal session model:
