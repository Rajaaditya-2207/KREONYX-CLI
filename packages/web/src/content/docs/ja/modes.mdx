...
:::caution
モードは、KREONYX CLI 設定の `agent` オプションを通じて設定されるようになりました。
...
これらを使用するか、KREONYX CLI 設定を通じて独自の設定を行います。
...
KREONYX CLI には 2 つの組み込みモードが付属しています。
...
- `edit` - 計画自体の詳細を示す `.kreonyx/plans/*.md` にあるファイルを除き、既存のファイルを変更できません
- `patch` - パッチを適用できません
...
- グローバル: `~/.config/kreonyx/modes/`
- プロジェクト: `.kreonyx/modes/`
...
温度が指定されていない場合、KREONYX CLI はモデル固有のデフォルトを使用します (通常、ほとんどのモデルでは 0、Qwen モデルでは 0.55)。
...
モードファイルをプロジェクト固有モードの場合は `.kreonyx/modes/` に、グローバルモードの場合は `~/.config/kreonyx/modes/` に作成します。
