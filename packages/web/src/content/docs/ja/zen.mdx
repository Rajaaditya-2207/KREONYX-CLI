---
title: KREONYX CLI Zen
description: KREONYX CLI によって提供されるモデルの厳選されたリスト。
---
...
KREONYX CLI Zen は、KREONYX CLI チームによって提供される、テストおよび検証されたモデルのリストです。

:::note
KREONYX CLI Zen は現在ベータ版です。
:::
Zen は KREONYX CLI の他のプロバイダーと同様に機能します。 KREONYX CLI Zen にログインすると、API キーを取得できます。これは **完全にオプション** であり、KREONYX CLI を使用するために使用する必要はありません。
...
:::tip
私たちは、KREONYX CLI で適切に動作するモデルとプロバイダーの選択されたグループをテストしました。
:::
...
KREONYX CLI Zen は、これらのモデルへのアクセスを可能にする AI ゲートウェイです。

---

## 仕組み

KREONYX CLI Zen は、KREONYX CLI の他のプロバイダーと同様に機能します。

1. **<a href={console}>KREONYX CLI Zen</a>** にログインし、請求内容を追加します
   詳細を確認し、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、KREONYX CLI Zen を選択して API キーを貼り付けます。
...
KREONYX CLI 設定の [model id](/docs/config/#models)
`kreonyx/<model-id>` 形式を使用します。たとえば、GPT 5.2 Codex の場合は、
設定で `kreonyx/gpt-5.2-codex` を使用してください。
...
- GLM 4.7 Free は期間限定で KREONYX CLI で入手できます。チームはこの時間を利用してフィードバックを収集し、モデルを改善します。
- Kim K2.5 Free は KREONYX CLI で期間限定で利用可能です。チームはこの時間を利用してフィードバックを収集し、モデルを改善します。
- MiniMax M2.1 Free は期間限定で KREONYX CLI で入手できます。チームはこの時間を利用してフィードバックを収集し、モデルを改善します。
- Big Pickle は、期間限定で KREONYX CLI で無料で利用できるステルスモデルです。チームはこの時間を利用してフィードバックを収集し、モデルを改善します。
...
私たちは次の目的で KREONYX CLI Zen を作成しました。
...
4. 他のコーディングエージェントとの併用を許可することで、**ロックイン**がなくなります。また、常に KREONYX CLI で他のプロバイダーも使用できるようにします。
