---
title: サーバー
description: HTTP 経由で KREONYX CLI サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`kreonyx serve` コマンドは、KREONYX CLI クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `kreonyx.local` |
...
HTTP 基本認証でサーバーを保護するには、`OPENCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `kreonyx` になるか、`OPENCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`kreonyx serve` と `kreonyx web` の両方に当てはまります。
...
`kreonyx` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
KREONYX CLI サーバーを使用して、プログラムで KREONYX CLI と対話します。
:::

`kreonyx serve` を実行してスタンドアロンサーバーを起動できます。すでに KREONYX CLI TUI を実行している場合でも、`kreonyx serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、KREONYX CLI [IDE](/docs/ide) プラグインによって使用されます。
...
KREONYX CLI サーバーは次の API を公開します。
