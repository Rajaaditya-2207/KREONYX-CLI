---
title: CLI
description: KREONYX CLI CLI のオプションとコマンド。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KREONYX CLI CLI は、引数なしで実行すると、デフォルトで [TUI](/docs/tui) を開始します。

```bash
kreonyx
```

ただし、このページに記載されているようにコマンドも受け入れます。これにより、KREONYX CLI をプログラム的に操作できるようになります。
...
KREONYX CLI ターミナルユーザーインターフェイスを開始します。
...
KREONYX CLI CLI には次のコマンドもあります。

---

### agent

KREONYX CLI のエージェントを管理します。
...
`serve` または `web` コマンドを使用して起動された、すでに実行中の KREONYX CLI バックエンドサーバーにターミナルを接続します。

```bash
kreonyx attach [url]
```

これにより、リモート KREONYX CLI バックエンドで TUI を使用できるようになります。例えば：
...
KREONYX CLI は [Models.dev](https://models.dev) のプロバイダーリストを利用しているため、`kreonyx auth login` を使用して、使用したいプロバイダーの API キーを構成できます。これは `~/.local/share/kreonyx/auth.json` に保存されます。

```bash
kreonyx auth login
```

KREONYX CLI が起動すると、認証情報ファイルからプロバイダーがロードされます。また、環境またはプロジェクト内の `.env` ファイルで定義されたキーがあるかどうかも確認します。
...
`--refresh` フラグを使用して、キャッシュされたモデルリストを更新します。これは、新しいモデルがプロバイダーに追加され、それを KREONYX CLI で確認したい場合に便利です。
...
プロンプトを直接渡して、非対話モードで kreonyx を実行します。
...
実行中の `kreonyx serve` インスタンスにアタッチして、実行ごとの MCP サーバーのコールドブート時間を回避することもできます。
...
| `--attach`   |             | 実行中の kreonyx サーバー (http://localhost:4096 など) に接続します。                    |
...
API アクセスのためにヘッドレス KREONYX CLI サーバーを起動します。完全な HTTP インターフェイスについては、[サーバーのドキュメント](/docs/server) を確認してください。

```bash
kreonyx serve
```

これにより、TUI インターフェイスを使用せずに kreonyx 機能への API アクセスを提供する HTTP サーバーが起動します。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `kreonyx`)。
...
KREONYX CLI セッションのトークンの使用状況とコストの統計を表示します。
...
JSON ファイルまたは KREONYX CLI 共有 URL からセッションデータをインポートします。

```bash
kreonyx import <file>
```

ローカルファイルまたは KREONYX CLI 共有 URL からインポートできます。
...
Web インターフェイスを使用してヘッドレス KREONYX CLI サーバーを起動します。

```bash
kreonyx web
```

これにより、HTTP サーバーが起動し、Web ブラウザが開き、Web インターフェイスを通じて KREONYX CLI にアクセスします。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `kreonyx`)。
...
KREONYX CLI をアンインストールし、関連ファイルをすべて削除します。
...
kreonyx を最新バージョンまたは特定のバージョンに更新します。
...
kreonyx CLI は次のグローバルフラグを受け取ります。
...
| `OPENCODE_SERVER_USERNAME`            | 文字列   | 基本認証ユーザー名 (デフォルト `kreonyx`) をオーバーライドします。 |
