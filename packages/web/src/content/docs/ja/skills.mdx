...
エージェントスキルにより、KREONYX CLI はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
KREONYX CLI は次の場所を検索します。

- プロジェクト設定: `.kreonyx/skills/<name>/SKILL.md`
- グローバル設定: `~/.config/kreonyx/skills/<name>/SKILL.md`
...
プロジェクトのローカルパスの場合、KREONYX CLI は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
途中で、一致する `skills/*/SKILL.md` を `.kreonyx/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。

グローバル定義は、`~/.config/kreonyx/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md`、および `~/.agents/skills/*/SKILL.md` からもロードされます。
...
次のように `.kreonyx/skills/git-release/SKILL.md` を作成します。
...
KREONYX CLI では、`skill` ツールの説明に利用可能なスキルがリストされています。
...
`kreonyx.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
