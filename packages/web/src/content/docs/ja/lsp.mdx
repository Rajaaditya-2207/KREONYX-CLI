---
title: LSPサーバー
description: KREONYX CLI は LSP サーバーと統合します。
---

KREONYX CLI は言語サーバープロトコル (LSP) と統合して、LLM がコードベースと対話できるようにします。診断を使用して LLM にフィードバックを提供します。

---

## 組み込み

KREONYX CLI には、一般的な言語用のいくつかの組み込み LSP サーバーが付属しています。
...
KREONYX CLI がファイルを開くと、次のようになります。
...
KREONYX CLI 設定の `lsp` セクションを通じて LSP サーバーをカスタマイズできます。
