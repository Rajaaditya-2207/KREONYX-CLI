---
title: 플러그인
description: KREONYX CLI를 확장하기 위해 자신만의 플러그인을 작성하세요.
---

플러그인은 다양한 이벤트와 사용자 정의 행동으로 후킹하여 kreonyx를 확장 할 수 있습니다. 플러그인을 만들 수 있습니다 새로운 기능을 추가, 외부 서비스와 통합, 또는 kreonyx의 기본 동작을 수정.
...
- `.kreonyx/plugins/` - 프로젝트 레벨 플러그인
- `~/.config/kreonyx/plugins/` - 글로벌 플러그인
...
**npm 플러그인**은 시작시 Bun을 사용하여 자동으로 설치됩니다. 패키지와 그들의 의존성은 `~/.cache/kreonyx/node_modules/`에서 캐시됩니다.
...
1. 글로벌 구성 (`~/.config/kreonyx/kreonyx.json`)
2. 프로젝트 구성 (`kreonyx.json`)
3. 글로벌 플러그인 디렉토리 (`~/.config/kreonyx/plugins/`)
4. 프로젝트 플러그인 디렉토리 (`.kreonyx/plugins/`)
...
kreonyx는 `bun install`를 시작합니다. 플러그인 및 도구가 가져올 수 있습니다.
...
- `client`: AI와 상호 작용을 위한 kreonyx SDK 클라이언트.
...
kreonyx를 확장하기 위해 사용할 수있는 플러그인의 몇 가지 예입니다.
...
:::note
kreonyx 데스크톱 앱을 사용하는 경우 응답이 준비되어 있거나 세션 오류가 있을 때 시스템 알림을 자동으로 보낼 수 있습니다.
:::
...
플러그인은 kreonyx에 사용자 정의 도구를 추가 할 수 있습니다 :
...
`tool` helper는 kreonyx가 호출 할 수있는 사용자 정의 도구를 만듭니다. Zod schema 기능을 가지고 도구 정의를 반환:

- `description`: 도구는 무엇을
- `args`: 도구의 인수에 대한 Zod 스키마
- `execute`: 도구가 호출될 때 실행되는 기능

사용자 정의 도구는 내장 도구와 함께 kreonyx를 사용할 수 있습니다.
