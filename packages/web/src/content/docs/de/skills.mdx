...
Agent Skills erlauben KREONYX CLI, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
KREONYX CLI sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft KREONYX CLI vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.kreonyx/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/kreonyx/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.kreonyx/skills/git-release/SKILL.md` zum Beispiel so:
...
KREONYX CLI listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `kreonyx.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `kreonyx.json`):
