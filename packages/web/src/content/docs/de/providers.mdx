---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in KREONYX CLI.
---

import config from "../../../../config.mjs"
export const console = config.console

KREONYX CLI verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer KREONYX CLI-Konfiguration.
...
in `~/.local/share/kreonyx/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem KREONYX CLI anpassen
config.
...
KREONYX CLI Zen ist eine Liste von Modellen, die vom KREONYX CLI-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit KREONYX CLI funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit KREONYX CLI Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie KREONYX CLI aus und gehen Sie zu [opencode.ai/auth](https://opencode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in KREONYX CLI und ist völlig optional.
...
So verwenden Sie Amazon Bedrock mit KREONYX CLI:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie KREONYX CLI ausführen:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY kreonyx

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile kreonyx

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX kreonyx
   ```
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `kreonyx.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in KREONYX CLI wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit KREONYX CLI ordnungsgemäß funktioniert.
   :::
...
5. Legen Sie Ihren Ressourcennamen als Umgebungsvariable fest:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX kreonyx
   ```
...
5. Legen Sie Ihren Ressourcennamen als Umgebungsvariable fest:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX kreonyx
   ```
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre KREONYX CLI-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `KREONYX CLI`, Bereiche: `api`
...
um das Token im KREONYX CLI-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
KREONYX CLI verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von KREONYX CLI
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`kreonyx.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [kreonyx-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `kreonyx.json`:
...
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": ["@gitlab/kreonyx-gitlab-plugin"]
}
```
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit KREONYX CLI:
...
So verwenden Sie Google Vertex AI mit KREONYX CLI:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie KREONYX CLI ausführen.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id kreonyx
   ```
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über KREONYX CLI konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
```jsonc title="~/.config/kreonyx/kreonyx.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "helicone": {
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [kreonyx-helicone-session](https://github.com/H2Shami/kreonyx-helicone-session)-Plugin, um jede KREONYX CLI-Konversation automatisch als Sitzung in Helicone zu protokollieren.

```bash
npm install -g kreonyx-helicone-session
```
...
```json title="kreonyx.json"
{
  "plugin": ["kreonyx-helicone-session"]
}
```

Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede KREONYX CLI-Konversation als separate Sitzung aufgeführt.
...
Sie können KREONYX CLI für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren

```json title="kreonyx.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Sie können KREONYX CLI über LM Studio für die Verwendung lokaler Modelle konfigurieren.

```json title="kreonyx.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
...
Sie können KREONYX CLI über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für KREONYX CLI konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/kreonyx).
:::

```json title="kreonyx.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "ollama": {
...
So verwenden Sie Ollama Cloud mit KREONYX CLI:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in KREONYX CLI.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in KREONYX CLI verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
KREONYX CLI Zen ist eine Liste getesteter und verifizierter Modelle, die vom KREONYX CLI-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>KREONYX CLI Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **KREONYX CLI Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren KREONYX CLI API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre KREONYX CLI-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre KREONYX CLI-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
3. Geben Sie Ihren JSON-Service-Account-Key ein.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' kreonyx
   ```
...
4. Legen Sie optional die Bereitstellung ID und die Ressourcengruppe fest:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group kreonyx
   ```
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit KREONYX CLI:
...
Sie können Modelle auch über Ihre KREONYX CLI-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit KREONYX CLI verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
3. Geben Sie Ihren API-Schlüssel für den Anbieter ein.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in kreonyx.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Erstellen oder aktualisieren Sie Ihre `kreonyx.json`-Datei in Ihrem Projektverzeichnis:

   ```json title="kreonyx.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="kreonyx.json" {9,11,17-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {
...
Mithilfe der `limit`-Felder kann KREONYX CLI erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `kreonyx auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die KREONYX CLI-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer KREONYX CLI-Konfiguration übereinstimmt.
