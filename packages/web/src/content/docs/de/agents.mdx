...
Es gibt zwei Arten von Agenten in KREONYX CLI; Primäragenten und Subagenten.
...
KREONYX CLI verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `kreonyx.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet KREONYX CLI modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale KREONYX CLI-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer KREONYX CLI-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [KREONYX CLI Zen](/docs/zen) verwenden, würden Sie `kreonyx/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `kreonyx models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/kreonyx).
:::
