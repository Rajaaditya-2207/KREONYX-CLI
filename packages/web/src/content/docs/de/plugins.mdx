---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um KREONYX CLI zu erweitern.
---

Mit Plugins können Sie KREONYX CLI erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von KREONYX CLI zu ändern.
...
- `.kreonyx/plugins/` – Plugins auf Projektebene
- `~/.config/kreonyx/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/kreonyx/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/kreonyx/kreonyx.json`)
2. Projektkonfiguration (`kreonyx.json`)
3. Globales Plugin-Verzeichnis (`~/.config/kreonyx/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.kreonyx/plugins/`)
...
KREONYX CLI führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein KREONYX CLI-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie KREONYX CLI erweitern können.
...
:::note
Wenn Sie die KREONYX CLI-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass KREONYX CLI `.env`-Dateien liest:
...
Plugins können KREONYX CLI auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das KREONYX CLI aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für KREONYX CLI zur Verfügung.
