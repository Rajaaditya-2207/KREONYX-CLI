...
Sie können externe Tools zu KREONYX CLI hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. KREONYX CLI unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [KREONYX CLI Config](https://opencode.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/kreonyx`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
KREONYX CLI übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird KREONYX CLI Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von KREONYX CLI beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `kreonyx mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert KREONYX CLI die Token sicher in `~/.local/share/kreonyx/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in KREONYX CLI verfügbar. Sie können sie also wie jedes andere Tool über die KREONYX CLI-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und KREONYX CLI mit Ihrem Sentry-Konto zu verbinden.
