---
title: Regeln
description: Lege eigene Anweisungen fuer kreonyx fest.
---

Du kannst kreonyx ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in kreonyx den Befehl `/init` aus.
...
So kann kreonyx sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht kreonyx sie zu erweitern.
...
kreonyx kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/kreonyx/AGENTS.md` speichern.
Diese gelten in allen kreonyx-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt KREONYX CLI dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/kreonyx/AGENTS.md` existiert)
...
Beim Start sucht kreonyx in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/kreonyx/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `kreonyx.json` oder in `~/.config/kreonyx/kreonyx.json` angeben.
...
kreonyx parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit kreonyx.json

Empfohlen ist das `instructions`-Feld in der `kreonyx.json`:
...
Du kannst kreonyx explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `kreonyx.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
