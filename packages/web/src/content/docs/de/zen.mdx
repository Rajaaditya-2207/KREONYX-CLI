---
title: Zen
description: Kuratierte Modellliste direkt von KREONYX CLI.
---
...
KREONYX CLI Zen ist eine vom KREONYX CLI-Team getestete und verifizierte Modellliste.

:::note
KREONYX CLI Zen befindet sich aktuell in der Beta.
:::

Zen funktioniert wie jeder andere Provider in KREONYX CLI.
Du meldest dich bei KREONYX CLI Zen an, holst dir deinen API-Key und nutzt ihn optional.
...
:::tip
Wir haben eine Auswahl aus Modellen und Providern getestet, die gut mit KREONYX CLI funktionieren.
:::
...
KREONYX CLI Zen ist ein AI-Gateway, das dir Zugriff auf genau diese Modelle gibt.

---

## Funktionsweise

KREONYX CLI Zen funktioniert wie jeder andere Provider in KREONYX CLI.

1. Melde dich bei **<a href={console}>KREONYX CLI Zen</a>** an, hinterlege Zahlungsdaten und kopiere deinen API-Key.
2. Fuehre in der TUI `/connect` aus, waehle KREONYX CLI Zen und fuege den API-Key ein.
...
Die [Model-ID](/docs/config/#models) in deiner KREONYX CLI-Konfiguration hat das Format `kreonyx/<model-id>`.
Fuer GPT 5.2 Codex verwendest du zum Beispiel `kreonyx/gpt-5.2-codex`.
...
Wir haben KREONYX CLI Zen entwickelt, um:
...
4. **Kein Lock-in** zu erzwingen, damit du Zen mit anderen Coding-Agents und KREONYX CLI weiter mit anderen Providern nutzen kannst
