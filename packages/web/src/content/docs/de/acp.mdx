...
description: Verwenden Sie KREONYX CLI in jedem ACP-kompatiblen Editor.
---

KREONYX CLI unterstützt [Agent Client Protocol](https://agentclientprotocol.com) oder (ACP), sodass Sie es direkt in kompatiblen Editoren und IDEs verwenden können.
...
Um KREONYX CLI über ACP zu verwenden, konfigurieren Sie Ihren Editor für die Ausführung des Befehls `kreonyx acp`.

Der Befehl startet KREONYX CLI als ACP-kompatiblen Unterprozess, der über stdio über JSON-RPC mit Ihrem Editor kommuniziert.
...
Um es zu öffnen, verwenden Sie den neuen Agenten „KREONYX CLI“ in der AI Chat Agent Selector.
...
Um KREONYX CLI als ACP-Agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim) zu verwenden, fügen Sie Folgendes zu Ihrer Neovim-Konfiguration hinzu:
...
Diese Konfiguration richtet CodeCompanion so ein, dass KREONYX CLI als ACP-Agent für den Chat verwendet wird.
...
KREONYX CLI funktioniert über ACP genauso wie im Terminal. Alle Funktionen werden unterstützt:
...
- MCP-Server, die in Ihrer KREONYX CLI-Konfiguration konfiguriert sind
