---
title: Server
description: Nutze den kreonyx-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `kreonyx serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein kreonyx-Client nutzen kann.
...
Der Benutzername ist standardmaessig `kreonyx`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `kreonyx serve` und `kreonyx web`.
...
Wenn du `kreonyx` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den kreonyx-Server, um kreonyx programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `kreonyx serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `kreonyx serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die KREONYX CLI-[IDE](/docs/ide)-Plugins.
...
Der kreonyx-Server stellt folgende APIs bereit.
