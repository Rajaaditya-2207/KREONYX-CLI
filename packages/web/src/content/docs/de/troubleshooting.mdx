...
Wenn KREONYX CLI Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `kreonyx --log-level DEBUG`.

---

## Speicher

kreonyx speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
KREONYX CLI Desktop startet im Hintergrund einen lokalen KREONYX CLI-Server (`kreonyx-cli`-Sidecar).
...
- Nur macOS: `KREONYX CLI`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
Wenn du Probleme mit KREONYX CLI hast:
