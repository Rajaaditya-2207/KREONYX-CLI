---
title: CLI
description: KREONYX CLI CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der KREONYX CLI CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
kreonyx
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit KREONYX CLI interagieren.
...
Starten Sie die Terminalbenutzeroberfläche KREONYX CLI.
...
Der KREONYX CLI CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für KREONYX CLI verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden KREONYX CLI-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
kreonyx attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend KREONYX CLI. Zum Beispiel:
...
KREONYX CLI wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `kreonyx auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/kreonyx/auth.json` gespeichert.

```bash
kreonyx auth login
```

Wenn KREONYX CLI startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in KREONYX CLI sehen möchten.
...
Führen Sie KREONYX CLI im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `kreonyx serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| `--attach`   |      | An einen laufenden KREONYX CLI-Server anschließen (z.B. http://localhost:4096)                         |
...
Starten Sie einen Headless-KREONYX CLI-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
kreonyx serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf KREONYX CLI-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `kreonyx`).
...
KREONYX CLI-Sitzungen verwalten.
...
Listen Sie alle KREONYX CLI-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre KREONYX CLI-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer KREONYX CLI-Freigabe-URL.

```bash
kreonyx import <file>
```

Sie können aus einer lokalen Datei oder einer KREONYX CLI-Freigabe-URL importieren.
...
Starten Sie einen Headless-KREONYX CLI-Server mit einer Weboberfläche.

```bash
kreonyx web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf KREONYX CLI zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `kreonyx`).
...
Deinstallieren Sie KREONYX CLI und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert KREONYX CLI auf die neueste Version oder eine bestimmte Version.
...
Der KREONYX CLI CLI akzeptiert die folgenden globalen Flags.
...
KREONYX CLI kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `kreonyx`) |
