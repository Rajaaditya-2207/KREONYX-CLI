---
title: Konfiguration
description: Verwenden Sie die Konfiguration KREONYX CLI JSON.
---

Sie können KREONYX CLI mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

KREONYX CLI unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
Wenn Ihre globale Konfiguration beispielsweise `theme: "kreonyx"` und `autoupdate: true` und Ihre Projektkonfiguration `model: "anthropic/claude-sonnet-4-5"` festlegt, umfasst die endgültige Konfiguration alle drei Einstellungen.
...
1. **Remote-Konfiguration** (von `.well-known/kreonyx`) – Organisationsstandards
2. **Globale Konfiguration** (`~/.config/kreonyx/kreonyx.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`OPENCODE_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`kreonyx.json` im Projekt) – projektspezifische Einstellungen
5. **`.kreonyx` Verzeichnisse** – Agenten, Befehle, Plugins
...
:::note
Die Verzeichnisse `.kreonyx` und `~/.config/kreonyx` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Organisationen können die Standardkonfiguration über den Endpunkt `.well-known/kreonyx` bereitstellen. Dieser wird automatisch abgerufen, wenn Sie sich bei einem Anbieter authentifizieren, der dies unterstützt.
...
Platzieren Sie Ihre globale KREONYX CLI-Konfiguration in `~/.config/kreonyx/kreonyx.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `kreonyx.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.

:::tip
Platzieren Sie die projektspezifische Konfiguration im Stammverzeichnis Ihres Projekts.
:::

Wenn KREONYX CLI gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Modi und Plugins genau wie das Standardverzeichnis `.kreonyx` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.kreonyx` geladen, sodass es deren Einstellungen **überschreiben** kann.
...
Sie können Servereinstellungen für die Befehle `kreonyx serve` und `kreonyx web` über die Option `server` konfigurieren.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres KREONYX CLI-Servers erkannt werden.
- `mdnsDomain` – Benutzerdefinierter Domänenname für den mDNS-Dienst. Standardmäßig ist `kreonyx.local`. Nützlich für die Ausführung mehrerer Instanzen im selben Netzwerk.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer KREONYX CLI-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht KREONYX CLI, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer KREONYX CLI-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/kreonyx/agents/` oder `.kreonyx/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift KREONYX CLI mit einer Warnung auf `"build"` zurück.

Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`kreonyx run`), Desktop-App und GitHub Aktion.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/kreonyx/commands/` oder `.kreonyx/commands/` definieren. [Mehr erfahren](/docs/commands).
...
KREONYX CLI lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
KREONYX CLI erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert KREONYX CLI mit benutzerdefinierten Tools, Hooks und Integrationen.

Platzieren Sie Ihre Plugin-Dateien in `.kreonyx/plugins/` oder `~/.config/kreonyx/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
...
Dies ist nützlich, wenn Sie KREONYX CLI darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
