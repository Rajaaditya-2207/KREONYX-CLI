...
description: Erstellen Sie Tools, die der LLM in KREONYX CLI aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von KREONYX CLI wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.kreonyx/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/kreonyx/tools/` platzieren.
...
```ts title=".kreonyx/tools/database.ts" {1}
...
```ts title=".kreonyx/tools/math.ts"
import { tool } from "@kreonyx/plugin"
...
```ts title=".kreonyx/tools/project.ts" {8}
import { tool } from "@kreonyx/plugin"
...
```python title=".kreonyx/tools/add.py"
import sys
...
```ts title=".kreonyx/tools/python-add.ts" {10}
import { tool } from "@kreonyx/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".kreonyx/tools/add.py")
