...
:::caution
Modi werden jetzt über die Option `agent` in der KREONYX CLI-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in KREONYX CLI können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die KREONYX CLI-Konfiguration.
...
KREONYX CLI verfügt über zwei integrierte Modi.
...
- `edit` – Vorhandene Dateien können nicht geändert werden, mit Ausnahme der Dateien unter `.kreonyx/plans/*.md`, um den Plan selbst detailliert darzustellen
...
Konfigurieren Sie Modi in Ihrer `kreonyx.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet KREONYX CLI modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale KREONYX CLI-Konfiguration als auch die projektspezifische Konfiguration.
...
Erstellen Sie Modusdateien in `.kreonyx/modes/` für projektspezifische Modi oder `~/.config/kreonyx/modes/` für globale Modi:
