...
Erstellen Sie `.kreonyx/commands/test.md`:

```md title=".kreonyx/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die KREONYX CLI-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem KREONYX CLI [config](/docs/config):

```json title="kreonyx.jsonc" {4-12}
{
...
- Global: `~/.config/kreonyx/commands/`
- Pro Projekt: `.kreonyx/commands/`

```markdown title="~/.config/kreonyx/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".kreonyx/commands/component.md"
---
...
```md title=".kreonyx/commands/create-file.md"
---
...
```md title=".kreonyx/commands/analyze-coverage.md"
---
...
```md title=".kreonyx/commands/review-changes.md"
---
...
```md title=".kreonyx/commands/review-component.md"
---
...
KREONYX CLI enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
