...
KREONYX CLI has a list of keybinds that you can customize through the KREONYX CLI config.
...
KREONYX CLI uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The KREONYX CLI desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `kreonyx.json`.
