---
title: Provedores
description: Usando qualquer provedor LLM no kreonyx.
---

import config from "../../../../config.mjs"
export const console = config.console

kreonyx usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do kreonyx.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/kreonyx/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do kreonyx.
...
KREONYX CLI Zen é uma lista de modelos fornecidos pela equipe do kreonyx que foram testados e verificados para funcionar bem com o kreonyx. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o KREONYX CLI Zen.
:::

1. Execute o comando `/connect` no TUI, selecione kreonyx e acesse [opencode.ai/auth](https://opencode.ai/auth).
...
Funciona como qualquer outro provedor no kreonyx e é completamente opcional.
...
Para usar o Amazon Bedrock com o kreonyx:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   Defina uma dessas variáveis de ambiente ao executar o kreonyx:
   ...
   Para configuração específica do projeto ou persistente, use `kreonyx.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no kreonyx não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o kreonyx funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do kreonyx.
...
2. Escolha seu método de autenticação:
   ...
   3. Nome: `KREONYX CLI`, Escopos: `api`
...
armazenar o token no armazenamento de autenticação do kreonyx.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
kreonyx usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o kreonyx
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `kreonyx.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [kreonyx-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configuração

Personalize através do `kreonyx.json`:
...
Para usar sua assinatura do GitHub Copilot com o kreonyx:
...
Para usar o Google Vertex AI com o kreonyx:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o kreonyx.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do kreonyx, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [kreonyx-helicone-session](https://github.com/H2Shami/kreonyx-helicone-session) para registrar automaticamente cada conversa do kreonyx como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do kreonyx listada como uma sessão separada.
...
Você pode configurar o kreonyx para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o kreonyx para usar modelos locais através do LM Studio.
...
Você pode configurar o kreonyx para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o kreonyx. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/kreonyx) para detalhes.
:::
...
Para usar o Ollama Cloud com o kreonyx:
...
3. Copie a chave da API para uso no kreonyx.
...
6. **Importante**: Antes de usar modelos em nuvem no kreonyx, você deve puxar as informações do modelo localmente:
...
KREONYX CLI Zen é uma lista de modelos testados e verificados fornecidos pela equipe do kreonyx. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>KREONYX CLI Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **KREONYX CLI Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do kreonyx.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do kreonyx.
...
5. Você também pode personalizá-los através da sua configuração do kreonyx. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o kreonyx:
...
Você também pode personalizar modelos através da sua configuração do kreonyx. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o kreonyx. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
1. Execute o comando `/connect` e role para baixo até **Outro**.
   ...
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no kreonyx.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `kreonyx.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o kreonyx entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `kreonyx auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do kreonyx e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do kreonyx.
