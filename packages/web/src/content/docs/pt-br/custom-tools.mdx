...
description: Crie ferramentas que o LLM pode chamar no kreonyx.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do kreonyx, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.kreonyx/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/kreonyx/tools/`.
