---
title: Plugins
description: Escreva seus próprios plugins para estender o kreonyx.
---

Plugins permitem que você estenda o kreonyx conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do kreonyx.
...
- `.kreonyx/plugins/` - Plugins em nível de projeto
- `~/.config/kreonyx/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/kreonyx/node_modules/`.
...
1. Configuração global (`~/.config/kreonyx/kreonyx.json`)
2. Configuração do projeto (`kreonyx.json`)
3. Diretório de plugins global (`~/.config/kreonyx/plugins/`)
4. Diretório de plugins do projeto (`.kreonyx/plugins/`)
...
O kreonyx executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do kreonyx para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o kreonyx.
...
:::note
Se você estiver usando o aplicativo desktop kreonyx, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o kreonyx de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao kreonyx:
...
O helper `tool` cria uma ferramenta personalizada que o kreonyx pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o kreonyx junto com as ferramentas integradas.
