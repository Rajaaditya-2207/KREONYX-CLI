...
Para depurar problemas com o kreonyx, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\kreonyx\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `kreonyx --log-level DEBUG`.

---

## Armazenamento

O kreonyx armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/kreonyx/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\kreonyx`
...
O kreonyx Desktop executa um servidor kreonyx local (o sidecar `kreonyx-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `KREONYX CLI` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/kreonyx/kreonyx.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\kreonyx\kreonyx.jsonc`
...
O kreonyx também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/kreonyx/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\kreonyx\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o kreonyx possa reconstruí-lo.

1. Saia completamente do kreonyx Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/kreonyx`
- **Linux**: exclua `~/.cache/kreonyx` (ou execute `rm -rf ~/.cache/kreonyx`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\kreonyx`

3. Reinicie o kreonyx Desktop.

---

### Corrigindo problemas de conexão com o servidor

O kreonyx Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `kreonyx.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.
...
No Windows, o kreonyx Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do kreonyx.

---

### Notificações não aparecendo

O kreonyx Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o kreonyx nas configurações do seu sistema operacional, e
...
1. Saia do kreonyx Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo kreonyx Desktop):

- `kreonyx.settings.dat` (URL do servidor padrão do desktop)
- `kreonyx.global.dat` e `kreonyx.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o kreonyx:
...
Para descobrir quais modelos você tem acesso, execute `kreonyx models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\kreonyx`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O kreonyx instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\kreonyx`

2. Reinicie o kreonyx para reinstalar os pacotes de provedores mais recentes

Isso forçará o kreonyx a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O kreonyx detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
