...
As habilidades do agente permitem que o kreonyx descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O kreonyx pesquisa nesses locais:

- Configuração do projeto: `.kreonyx/skills/<name>/SKILL.md`
- Configuração global: `~/.config/kreonyx/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o kreonyx sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.kreonyx/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/kreonyx/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.kreonyx/skills/git-release/SKILL.md` assim:
...
O kreonyx lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `kreonyx.json`:
...
**Para agentes embutidos** (em `kreonyx.json`):
