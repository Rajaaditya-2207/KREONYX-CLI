---
title: Servidor
description: Interaja com o servidor kreonyx via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `kreonyx serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente kreonyx pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `kreonyx.local` |
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `kreonyx`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `kreonyx serve` quanto ao `kreonyx web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password kreonyx serve
```
...
Quando você executa `kreonyx`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor kreonyx para interagir com o kreonyx programaticamente.
:::

Essa arquitetura permite que o kreonyx suporte múltiplos clientes e permite que você interaja com o kreonyx programaticamente.

Você pode executar `kreonyx serve` para iniciar um servidor autônomo. Se você tiver o TUI do kreonyx em execução, `kreonyx serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do kreonyx [IDE](/docs/ide).
...
O servidor kreonyx expõe as seguintes APIs.
