---
title: CLI
description: Opções e comandos da CLI do kreonyx.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do kreonyx, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
kreonyx
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o kreonyx programaticamente.
...
Inicie a interface de usuário do terminal do kreonyx.
...
A CLI do kreonyx também possui os seguintes comandos.

---

### agent

Gerencie agentes para o kreonyx.
...
Anexe um terminal a um servidor backend do kreonyx já em execução, iniciado via comandos `serve` ou `web`.

```bash
kreonyx attach [url]
```

Isso permite usar o TUI com um backend kreonyx remoto. Por exemplo:
...
O kreonyx é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `kreonyx auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/kreonyx/auth.json`.

```bash
kreonyx auth login
```

Quando o kreonyx é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no kreonyx.
...
Execute o kreonyx em modo não interativo passando um prompt diretamente.
...
```bash "kreonyx run"
kreonyx run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `kreonyx serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
kreonyx run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| `--attach`   |       | Anexe a um servidor kreonyx em execução (por exemplo, http://localhost:4096) |
...
Inicie um servidor kreonyx sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
kreonyx serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do kreonyx sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `kreonyx`).
...
Liste todas as sessões do kreonyx.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do kreonyx.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do kreonyx.

```bash
kreonyx import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do kreonyx.
...
Inicie um servidor kreonyx sem cabeça com uma interface web.

```bash
kreonyx web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o kreonyx através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `kreonyx`).
...
Desinstale o kreonyx e remova todos os arquivos relacionados.
...
Atualiza o kreonyx para a versão mais recente ou uma versão específica.
...
A CLI do kreonyx aceita as seguintes flags globais.
...
O kreonyx pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `kreonyx`) |
