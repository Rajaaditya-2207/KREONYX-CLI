...
Você pode adicionar ferramentas externas ao kreonyx usando o _Model Context Protocol_, ou MCP. O kreonyx suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do kreonyx](https://opencode.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/kreonyx`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O kreonyx lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o kreonyx irá:
...
Se o servidor requer autenticação, o kreonyx solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `kreonyx mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o kreonyx armazenará os tokens de forma segura em `~/.local/share/kreonyx/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no kreonyx, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do kreonyx como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o kreonyx à sua conta do Sentry.
