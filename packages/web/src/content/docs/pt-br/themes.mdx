...
Com o kreonyx, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o kreonyx usa nosso próprio tema `kreonyx`.
...
O kreonyx vem com vários temas embutidos.
...
- Querem que o kreonyx corresponda à aparência do seu terminal
...
O kreonyx suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/kreonyx/themes/*.json` ou `$XDG_CONFIG_HOME/kreonyx/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.kreonyx/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.kreonyx/themes/*.json`
