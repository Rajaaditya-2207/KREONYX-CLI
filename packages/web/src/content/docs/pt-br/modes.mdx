...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do kreonyx. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no kreonyx permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do kreonyx.
...
O kreonyx vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.kreonyx/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `kreonyx.json`:
...
- Global: `~/.config/kreonyx/modes/`
- Projeto: `.kreonyx/modes/`

```markdown title="~/.config/kreonyx/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Se nenhuma temperatura for especificada, o kreonyx usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do kreonyx quanto para a configuração específica do projeto.
...
Crie arquivos de modo em `.kreonyx/modes/` para modos específicos do projeto ou `~/.config/kreonyx/modes/` para modos globais:

```markdown title=".kreonyx/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/kreonyx/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
