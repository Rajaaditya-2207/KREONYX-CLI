---
title: Formatadores
description: O kreonyx usa formatadores específicos de linguagem.
---

O kreonyx formata automaticamente arquivos após serem escritos ou editados usando formatadores específicos de linguagem. Isso garante que o código gerado siga os estilos de código do seu projeto.

---

## Integrados

O kreonyx vem com vários formatadores integrados para linguagens e frameworks populares. Abaixo está uma lista dos formatadores, extensões de arquivo suportadas e comandos ou opções de configuração necessárias.
...
Portanto, se seu projeto tiver `prettier` em seu `package.json`, o kreonyx o usará automaticamente.

---

## Como funciona

Quando o kreonyx escreve ou edita um arquivo, ele:
...
Você pode personalizar os formatadores através da seção `formatter` em sua configuração do kreonyx.
