...
Create `.kreonyx/commands/test.md`:
...
You can add custom commands through the KREONYX CLI config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your KREONYX CLI [config](/docs/config):
...
kreonyx includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
