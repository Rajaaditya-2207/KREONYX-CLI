...
There are two types of agents in KREONYX CLI; primary agents and subagents.
...
KREONYX CLI comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
KREONYX CLI comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

KREONYX CLI comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `kreonyx.json` config file:
...
If no temperature is specified, KREONYX CLI uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global KREONYX CLI config and the project specific config.
...
The model ID in your KREONYX CLI config uses the format `provider/model-id`. For example, if you're using [KREONYX CLI Zen](/docs/zen), you would use `kreonyx/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `kreonyx models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/kreonyx).
:::
