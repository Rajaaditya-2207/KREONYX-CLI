...
:::caution
Modes are now configured through the `agent` option in the kreonyx config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in kreonyx allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the kreonyx config.
...
kreonyx comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.kreonyx/plans/*.md` to detail the plan itself
...
Configure modes in your `kreonyx.json` config file:
...
If no temperature is specified, kreonyx uses model-specific defaults (typically 0 for most models, 0.55 for Qwen models).
...
both the global kreonyx config and the project specific config.
...
Create mode files in `.kreonyx/modes/` for project-specific modes or `~/.config/kreonyx/modes/` for global modes:
