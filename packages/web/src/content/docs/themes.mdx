...
With KREONYX CLI you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, KREONYX CLI uses our own `kreonyx` theme.
...
KREONYX CLI comes with several built-in themes.
...
- Want KREONYX CLI to match their terminal's appearance
...
KREONYX CLI supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/kreonyx/themes/*.json` or `$XDG_CONFIG_HOME/kreonyx/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.kreonyx/themes/*.json`
4. **Current working directory** - Defined in `./.kreonyx/themes/*.json`
