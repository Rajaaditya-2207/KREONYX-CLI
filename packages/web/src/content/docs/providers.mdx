---
title: Providers
description: Using any LLM provider in KREONYX CLI.
---

import config from "../../../config.mjs"
export const console = config.console

KREONYX CLI uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your KREONYX CLI config.
...
You can customize the providers through the `provider` section in your KREONYX CLI
config.
...
KREONYX CLI Zen is a list of models provided by the KREONYX CLI team that have been
tested and verified to work well with KREONYX CLI. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with KREONYX CLI Zen.
:::

1. Run the `/connect` command in the TUI, select kreonyx, and head to [opencode.ai/auth](https://opencode.ai/auth).
...
It works like any other provider in KREONYX CLI and is completely optional to use.
...
To use Amazon Bedrock with KREONYX CLI:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running kreonyx:
   ...
   For project-specific or persistent configuration, use `kreonyx.json`:
...
:::info
Using your Claude Pro/Max subscription in KREONYX CLI is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for kreonyx to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your kreonyx config.
...
KREONYX CLI uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock KREONYX CLI
to only use your own GitLab-hosted instance, add the following to your
`kreonyx.json` file. It is also recommended to disable session sharing.
...
More documentation on [kreonyx-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) homepage.
...
To use your GitHub Copilot subscription with kreonyx:
...
To use Google Vertex AI with KREONYX CLI:
...
In the event you see a feature or model from Helicone that isn't configured automatically through kreonyx, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [kreonyx-helicone-session](https://github.com/H2Shami/kreonyx-helicone-session) plugin to automatically log each KREONYX CLI conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each KREONYX CLI conversation listed as a separate session.
...
You can configure kreonyx to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure kreonyx to use local models through LM Studio.
...
You can configure kreonyx to use local models through Ollama.

:::tip
Ollama can automatically configure itself for KREONYX CLI. See the [Ollama integration docs](https://docs.ollama.com/integrations/kreonyx) for details.
:::
...
To use Ollama Cloud with KREONYX CLI:
...
3. Copy the API key for use in KREONYX CLI.
...
6. **Important**: Before using cloud models in KREONYX CLI, you must pull the model information locally:
...
KREONYX CLI Zen is a list of tested and verified models provided by the KREONYX CLI team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>KREONYX CLI Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **KREONYX CLI Zen**.

   ```txt
   /connect
   ```

3. Enter your KREONYX CLI API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your kreonyx config.
...
5. You can also customize them through your kreonyx config. Here's an example of specifying a provider
...
You can also customize models through your kreonyx config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with kreonyx. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `kreonyx.json` file in your project directory:
...
The `limit` fields allow KREONYX CLI to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `kreonyx auth list` to see if the credentials
...
2. For custom providers, check the kreonyx config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your kreonyx config.
