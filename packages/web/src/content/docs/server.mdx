---
title: Server
description: Interact with kreonyx server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `kreonyx serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an kreonyx client can use.
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `kreonyx`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `kreonyx serve` and `kreonyx web`.
...
When you run `kreonyx` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the kreonyx server to interact with kreonyx programmatically.
:::

This architecture lets kreonyx support multiple clients and allows you to interact with kreonyx programmatically.

You can run `kreonyx serve` to start a standalone server. If you have the
kreonyx TUI running, `kreonyx serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the KREONYX CLI [IDE](/docs/ide) plugins.
...
The kreonyx server exposes the following APIs.
