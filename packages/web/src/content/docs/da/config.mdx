---
title: Konfiguration
description: Ved at bruge KREONYX CLI JSON konfig.
---

Du kan konfigurere KREONYX CLI ved hjælp af en JSON-konfigurationsfil.

---

## Format

KREONYX CLI understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis dine globale konfigurationssæt `theme: "kreonyx"` og `autoupdate: true`, og dine projektkonfigurationssæt `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfiguration integrere alle tre indstillinger.
...
1. **Fjernkonfiguration** (fra `.well-known/kreonyx`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/kreonyx/kreonyx.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OPENCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`kreonyx.json` i projekt) - projektspecifikke indstillinger
5. **`.kreonyx` mapper** - agent, kommandoer, plugins
...
:::note
`.kreonyx` og `~/.config/kreonyx` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/kreonyx`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale KREONYX CLI-konfiguration i `~/.config/kreonyx/kreonyx.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `kreonyx.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når KREONYX CLI starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
modes og plugins ligesom standard `.kreonyx` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.kreonyx` mapper, så den **kan tilsidesætte** deres indstillinger.
...
Du kan konfigurere serverindstillinger for kommandoerne `kreonyx serve` og `kreonyx web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din KREONYX CLI-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `kreonyx.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din KREONYX CLI-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger KREONYX CLI at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din KREONYX CLI-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/kreonyx/agents/` eller `.kreonyx/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil KREONYX CLI falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`kreonyx run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/kreonyx/commands/` eller `.kreonyx/commands/`. [Learn more here](/docs/commands).
...
KREONYX CLI vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard kreonyx **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide KREONYX CLI med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.kreonyx/plugins/` eller `~/.config/kreonyx/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse KREONYX CLI til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
