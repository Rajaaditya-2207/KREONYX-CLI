---
title: CLI
description: KREONYX CLI CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KREONYX CLI CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
kreonyx
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med KREONYX CLI programmatisk.
...
Start KREONYX CLI terminalbrugergrænsefladen.
...
KREONYX CLI CLI har også følgende kommandoer.

---

### agent

Administrer agenter i KREONYX CLI.
...
Tilslut en terminal til en allerede kørende KREONYX CLI backend-server startet via `serve` eller `web` kommandoer.

```bash
kreonyx attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern KREONYX CLI backend. For eksempel:
...
KREONYX CLI drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `kreonyx auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/kreonyx/auth.json`.

```bash
kreonyx auth login
```

Når KREONYX CLI starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i KREONYX CLI.
...
Kør kreonyx i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `kreonyx serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| `--attach`   |      | Tilslut til en kørende kreonyx-server (f.eks. http://localhost:4096)               |
...
Start en hovedløs KREONYX CLI-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
kreonyx serve
```

Dette starter en HTTP-server, der giver API-adgang til kreonyx-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `kreonyx`).
...
Administrator KREONYX CLI sessionsholder.
...
Liste over alle KREONYX CLI sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine KREONYX CLI-sessioner.
...
Importer sessionsdata fra en JSON fil eller KREONYX CLI del URL.

```bash
kreonyx import <file>
```

Du kan importere fra en lokal fil eller en KREONYX CLI share URL.
...
Start en hovedløs KREONYX CLI-server med en webgrænseflade.

```bash
kreonyx web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til KREONYX CLI via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `kreonyx`).
...
Afinstaller KREONYX CLI og fjern alle relaterede filer.
...
Opdaterer KREONYX CLI til den nyeste version eller en specifik version.
...
kreonyx CLI tager følgende globale flag.
...
KREONYX CLI kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `kreonyx`) |
