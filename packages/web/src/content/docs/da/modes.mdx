...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i kreonyx-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i kreonyx giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem kreonyx-konfigurationen.
...
kreonyx leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.kreonyx/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `kreonyx.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger kreonyx modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale kreonyx-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.kreonyx/modes/` for projektspecifikke tilstande eller `~/.config/kreonyx/modes/` for globale tilstande:
