...
For at fejlfinde problemer med KREONYX CLI, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\kreonyx\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `kreonyx --log-level DEBUG`.

---

## Lagring

kreonyx gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/kreonyx/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\kreonyx`
...
KREONYX CLI Desktop kører en lokal KREONYX CLI-server (`kreonyx-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `KREONYX CLI`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/kreonyx/kreonyx.jsonc` (eller `~/.config/kreonyx/kreonyx.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/kreonyx/kreonyx.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\kreonyx\kreonyx.jsonc`
...
KREONYX CLI kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/kreonyx/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\kreonyx\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så KREONYX CLI kan genopbygge den.

1. Afslut KREONYX CLI Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/kreonyx`
- **Linux**: slet `~/.cache/kreonyx` (eller kør `rm -rf ~/.cache/kreonyx`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\kreonyx`

3. Start KREONYX CLI Desktop på ny.

---

### Løs problemer med serverforbindelse

KREONYX CLI Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `kreonyx.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver KREONYX CLI Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i KREONYX CLI.

---

### Meddelelser vises ikke

KREONYX CLI Desktop viser kun systemvarsler når:

- varsler er aktiveret for KREONYX CLI i dine OS-indstillinger, og
...
1. Afslut KREONYX CLI Desktop.
2. Find og slet disse filer (de findes i KREONYX CLI Desktop-appens datamappe):

- `kreonyx.settings.dat` (skrivebordsstandardserver URL)
- `kreonyx.global.dat` og `kreonyx.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med KREONYX CLI:
...
For at finde ud af hvilke modeller du har adgang til, kør `kreonyx models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\kreonyx`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. kreonyx installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\kreonyx`

2. Start kreonyx på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge kreonyx til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
kreonyx vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
