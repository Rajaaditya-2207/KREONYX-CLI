...
Med KREONYX CLI kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger KREONYX CLI vårt eget `kreonyx`-tema.
...
KREONYX CLI kommer med flere innebygde temaer.
...
- Vil at KREONYX CLI skal matche terminalens utseende
...
KREONYX CLI støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/kreonyx/themes/*.json` eller `$XDG_CONFIG_HOME/kreonyx/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.kreonyx/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.kreonyx/themes/*.json`
