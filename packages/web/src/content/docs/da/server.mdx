---
title: Server
description: Interager med kreonyx-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `kreonyx serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en kreonyx-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `kreonyx.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `kreonyx`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `kreonyx serve` og `kreonyx web`.
...
Når du kører `kreonyx` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug kreonyx-serveren til at interagere med kreonyx programmatisk.
:::

Denne arkitektur lader kreonyx understøtte flere klienter og lader dig interagere med kreonyx programmatisk.

Du kan køre `kreonyx serve` for at starte en selvstændig server. Hvis du har
kreonyx TUI kørende, vil `kreonyx serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af KREONYX CLI [IDE](/docs/ide) plugins.
...
KREONYX CLI-serveren viser følgende API'er.
