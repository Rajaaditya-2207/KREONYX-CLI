---
title: Plugins
description: Skriv dine egne plugins for at udvide KREONYX CLI.
---

Plugins giver dig mulighed for at udvide KREONYX CLI ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre KREONYX CLIs standardadfærd.
...
- `.kreonyx/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/kreonyx/node_modules/`.
...
1. Global konfiguration (`~/.config/kreonyx/kreonyx.json`)
2. Projektkonfiguration (`kreonyx.json`)
3. Global plugin-mappe (`~/.config/kreonyx/plugins/`)
4. Projekt plugin bibliotek (`.kreonyx/plugins/`)
...
KREONYX CLI kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En kreonyx SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide kreonyx.
...
:::note
Hvis du bruger KREONYX CLI desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå kreonyx i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til kreonyx:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som kreonyx kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for kreonyx sammen med indbyggede værktøjer.
