...
description: Brug KREONYX CLI i enhver ACP-kompatibel editor.
---

KREONYX CLI understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge KREONYX CLI via ACP, konfigurer din editor til at køre kommandoen `kreonyx acp`.

Kommandoen starter KREONYX CLI som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'KREONYX CLI'-agent i AI Chat-agentvælgeren.
...
For at bruge KREONYX CLI som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge KREONYX CLI som ACP-agent til chat.
...
KREONYX CLI fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din KREONYX CLI-konfiguration
