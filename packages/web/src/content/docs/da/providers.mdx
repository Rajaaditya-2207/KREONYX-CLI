---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i KREONYX CLI.
---

import config from "../../../../config.mjs"
export const console = config.console

KREONYX CLI bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i KREONYX CLI-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/kreonyx/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i KREONYX CLI-konfigurationen.
...
KREONYX CLI Zen er en liste over modeller leveret af KREONYX CLI-teamet, som er blevet testet og verificeret til at fungere godt med KREONYX CLI. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med KREONYX CLI Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg kreonyx og gå til [opencode.ai/auth](https://opencode.ai/auth).
...
Det fungerer som alle andre udbydere i KREONYX CLI og er helt valgfrit at bruge.
...
Sådan bruger du Amazon Bedrock med KREONYX CLI:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører kreonyx:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `kreonyx.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i KREONYX CLI understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at kreonyx skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem kreonyx-konfigurationen.
...
gemme tokenet i kreonyx auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
KREONYX CLI bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse KREONYX CLI
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`kreonyx.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [kreonyx-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `kreonyx.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med KREONYX CLI:
...
Sådan bruger du Google Vertex AI med KREONYX CLI:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem kreonyx, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [kreonyx-helicone-session](https://github.com/H2Shami/kreonyx-helicone-session) plugin for automatisk at logge hver KREONYX CLI-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver KREONYX CLI-samtale opført som en separat session.
...
Du kan konfigurere kreonyx til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere kreonyx til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere kreonyx til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for KREONYX CLI. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/kreonyx) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med KREONYX CLI:
...
3. Kopier API-nøglen til brug i KREONYX CLI.
...
6. **Vigtigt**: Før du bruger skymodeller i KREONYX CLI, skal du hente modelinformationen lokalt:
...
KREONYX CLI Zen er en liste over testede og verificerede modeller leveret af KREONYX CLI-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>KREONYX CLI Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **KREONYX CLI Zen**.

   ```txt
   /connect
   ```

3. Indtast KREONYX CLI API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem kreonyx-konfigurationen.
...
5. Du kan også tilpasse dem gennem kreonyx-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med KREONYX CLI:
...
Du kan også tilpasse modeller gennem kreonyx-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med kreonyx. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `kreonyx.json`-filen i projektkataloget:
...
`limit`-felterne lader KREONYX CLI forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `kreonyx auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek kreonyx-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i kreonyx-konfigurationen.
