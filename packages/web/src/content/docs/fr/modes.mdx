...
:::caution
Les modes sont désormais configurés via l'option `agent` dans la configuration kreonyx.
L’option `mode` est désormais obsolète. [En savoir plus](/docs/agents).
:::

Les modes dans kreonyx vous permettent de personnaliser le comportement, les outils et les prompts pour différents cas d'utilisation.

Il est livré avec deux modes intégrés : **build** et **plan**. Vous pouvez personnaliser ceux-ci ou configurez les vôtres via la configuration kreonyx.
...
kreonyx est livré avec deux modes intégrés.
...
- `edit` - Impossible de modifier les fichiers existants, à l'exception des fichiers situés à `.kreonyx/plans/*.md` pour détailler le plan lui-même
...
Configurez les modes dans votre fichier de configuration `kreonyx.json` :
...
Si aucune température n'est spécifiée, kreonyx utilise les valeurs par défaut spécifiques au modèle (généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen).
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Donc ça marche pour à la fois la configuration globale kreonyx et la configuration spécifique au projet.
...
Créez des fichiers de mode dans `.kreonyx/modes/` pour les modes spécifiques au projet ou `~/.config/kreonyx/modes/` pour les modes globaux :
