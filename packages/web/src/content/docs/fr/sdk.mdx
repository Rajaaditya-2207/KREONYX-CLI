---
title: SDK
description: Client JS de type sécurisé pour le serveur kreonyx.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK kreonyx JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler kreonyx par programme.
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `kreonyx.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${kreonyx.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de kreonyx, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@kreonyx/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*kreonyx" },
})
