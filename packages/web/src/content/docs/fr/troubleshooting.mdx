...
Pour déboguer les problèmes avec KREONYX CLI, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\kreonyx\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `kreonyx --log-level DEBUG`.

---

## Stockage

kreonyx stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/kreonyx/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\kreonyx`
...
KREONYX CLI Desktop exécute un serveur KREONYX CLI local (le side-car `kreonyx-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `KREONYX CLI` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** : `~/.config/kreonyx/kreonyx.jsonc` (ou `~/.config/kreonyx/kreonyx.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/kreonyx/kreonyx.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\kreonyx\kreonyx.jsonc`
...
KREONYX CLI peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/kreonyx/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\kreonyx\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que KREONYX CLI puisse le reconstruire.

1. Quittez complètement KREONYX CLI Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/kreonyx`
- **Linux** : supprimez `~/.cache/kreonyx` (ou exécutez `rm -rf ~/.cache/kreonyx`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\kreonyx`

3. Redémarrez le bureau KREONYX CLI.

---

### Résoudre les problèmes de connexion au serveur

KREONYX CLI Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `kreonyx.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
...
Sur Windows, KREONYX CLI Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de KREONYX CLI.

---

### Les notifications ne s'affichent pas

KREONYX CLI Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour KREONYX CLI dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau KREONYX CLI.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application KREONYX CLI Desktop) :

- `kreonyx.settings.dat` (serveur de bureau par défaut URL)
- `kreonyx.global.dat` et `kreonyx.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec KREONYX CLI :
...
3. Assurez-vous d'avoir la dernière version avec `kreonyx upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `kreonyx models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\kreonyx`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. kreonyx installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\kreonyx`

2. Redémarrez kreonyx pour réinstaller les derniers packages du fournisseur

Cela forcera kreonyx à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
kreonyx détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
