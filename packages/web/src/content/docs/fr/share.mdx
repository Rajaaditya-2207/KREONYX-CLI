---
title: Partager
description: Partagez vos conversations KREONYX CLI.
---

La fonctionnalité de partage de KREONYX CLI vous permet de créer des liens publics vers vos conversations KREONYX CLI, afin que vous puissiez collaborer avec vos coéquipiers ou obtenir de l'aide d'autres personnes.
...
Lorsque vous partagez une conversation, KREONYX CLI :
...
KREONYX CLI prend en charge trois modes de partage qui contrôlent la manière dont les conversations sont partagées :

---

### Manuel (par défaut)

Par défaut, KREONYX CLI utilise le mode de partage manuel. Les sessions ne sont pas partagées automatiquement, mais vous pouvez les partager manuellement à l'aide de la commande `/share` :
...
Pour appliquer cela au sein de votre équipe pour un projet donné, ajoutez-le au `kreonyx.json` dans votre projet et enregistrez-vous dans Git.
...
[En savoir plus](/docs/enterprise) sur l'utilisation de kreonyx dans votre organisation.
