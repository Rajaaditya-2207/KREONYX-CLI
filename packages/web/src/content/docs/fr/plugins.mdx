---
title: Plugins
description: Écrivez vos propres plugins pour étendre KREONYX CLI.
---

Les plugins vous permettent d'étendre KREONYX CLI en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de KREONYX CLI.
...
- `.kreonyx/plugins/` - Plugins au niveau du projet
- `~/.config/kreonyx/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/kreonyx/node_modules/`.
...
1. Configuration globale (`~/.config/kreonyx/kreonyx.json`)
2. Configuration du projet (`kreonyx.json`)
3. Répertoire global des plugins (`~/.config/kreonyx/plugins/`)
4. Répertoire des plugins du projet (`.kreonyx/plugins/`)
...
KREONYX CLI exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK kreonyx pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre kreonyx.
...
:::note
Si vous utilisez l'application de bureau KREONYX CLI, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez kreonyx de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à kreonyx :
...
L'assistant `tool` crée un outil personnalisé que kreonyx peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour kreonyx aux côtés des outils intégrés.
