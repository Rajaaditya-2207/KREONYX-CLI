---
title: Configuration
description: Utilisation de la configuration JSON KREONYX CLI.
---

Vous pouvez configurer KREONYX CLI à l'aide d'un fichier de configuration JSON.

---

## Format

KREONYX CLI prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).

```jsonc title="kreonyx.jsonc"
{
...
Par exemple, si votre configuration globale définit `theme: "kreonyx"` et `autoupdate: true` et que la configuration de votre projet définit `model: "anthropic/claude-sonnet-4-5"`, la configuration finale inclura les trois paramètres.
...
1. **Configuration distante** (à partir de `.well-known/kreonyx`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/kreonyx/kreonyx.json`) - préférences utilisateur
3. **Configuration personnalisée** (`OPENCODE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`kreonyx.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.kreonyx`** - agents, commandes, plugins
...
:::note
Les répertoires `.kreonyx` et `~/.config/kreonyx` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/kreonyx`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
```json title="Remote config from .well-known/kreonyx"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Placez votre configuration globale KREONYX CLI dans `~/.config/kreonyx/kreonyx.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les thèmes, les fournisseurs ou les raccourcis clavier.
...
Ajoutez `kreonyx.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.

:::tip
Placez la configuration spécifique au projet à la racine de votre projet.
:::

Lorsque KREONYX CLI démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `OPENCODE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.kreonyx`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.kreonyx`, il **peut donc remplacer** leurs paramètres.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `kreonyx serve` et `kreonyx web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur KREONYX CLI.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `kreonyx.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration KREONYX CLI via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, KREONYX CLI essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez configurer le thème que vous souhaitez utiliser dans votre configuration KREONYX CLI via l'option `theme`.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/kreonyx/agents/` ou `.kreonyx/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, KREONYX CLI reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`kreonyx run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/kreonyx/commands/` ou `.kreonyx/commands/`. [En savoir plus ici](/docs/commands).
...
KREONYX CLI téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, kreonyx **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent KREONYX CLI avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.kreonyx/plugins/` ou `~/.config/kreonyx/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.

```json title="kreonyx.json"
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": ["kreonyx-helicone-session", "@my-org/custom-plugin"]
}
```
...
Ceci est utile lorsque vous souhaitez restreindre KREONYX CLI à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
