...
Les compétences d'agent permettent à KREONYX CLI de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
KREONYX CLI recherche ces emplacements :

- Configuration du projet : `.kreonyx/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/kreonyx/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, KREONYX CLI remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.kreonyx/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/kreonyx/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.kreonyx/skills/git-release/SKILL.md` comme ceci :
...
KREONYX CLI répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `kreonyx.json` :
...
**Pour les agents intégrés** (dans `kreonyx.json`) :
