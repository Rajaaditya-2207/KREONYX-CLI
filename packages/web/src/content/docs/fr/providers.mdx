---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en KREONYX CLI.
---

import config from "../../../../config.mjs"
export const console = config.console

KREONYX CLI utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration KREONYX CLI.
...
en `~/.local/share/kreonyx/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre KREONYX CLI
configuration.
...
KREONYX CLI Zen est une liste de modèles fournis par l'équipe KREONYX CLI qui ont été
testé et vérifié pour fonctionner correctement avec KREONYX CLI. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par KREONYX CLI Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez kreonyx et dirigez-vous vers [opencode.ai/auth](https://opencode.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans KREONYX CLI et son utilisation est totalement facultative.
...
Pour utiliser Amazon Bedrock avec KREONYX CLI :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de kreonyx :
...
Pour une configuration spécifique au projet ou persistante, utilisez `kreonyx.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans KREONYX CLI n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que kreonyx fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration kreonyx.
...
pour stocker le jeton dans le stockage d'authentification kreonyx.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
KREONYX CLI utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller KREONYX CLI
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `kreonyx.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [kreonyx-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configuration

Personnalisez via `kreonyx.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec kreonyx :
...
Pour utiliser Google Vertex AI avec KREONYX CLI :
...
Définissez-les lors de l'exécution de kreonyx.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via kreonyx, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [kreonyx-helicone-session](https://github.com/H2Shami/kreonyx-helicone-session) pour enregistrer automatiquement chaque conversation KREONYX CLI en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation KREONYX CLI répertoriée comme une session distincte.
...
Vous pouvez configurer kreonyx pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer kreonyx pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer kreonyx pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour KREONYX CLI. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/kreonyx) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec KREONYX CLI :
...
3. Copiez la clé API à utiliser dans KREONYX CLI.
...
6. **Important** : Avant d'utiliser des modèles cloud dans KREONYX CLI, vous devez extraire les informations du modèle localement :
...
KREONYX CLI Zen est une liste de modèles testés et vérifiés fournie par l'équipe KREONYX CLI. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>KREONYX CLI Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **KREONYX CLI Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé KREONYX CLI API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration kreonyx.
...
5. Vous pouvez également les personnaliser via votre configuration kreonyx. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration kreonyx. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec kreonyx. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `kreonyx.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à KREONYX CLI de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `kreonyx auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration kreonyx et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration kreonyx.
