...
Créez `.kreonyx/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration KREONYX CLI ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre KREONYX CLI [config](/docs/config) :
...
kreonyx comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
