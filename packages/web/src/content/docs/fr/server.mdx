---
title: Serveur
description: Interagissez avec le serveur kreonyx via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `kreonyx serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client kreonyx peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `kreonyx.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `kreonyx`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `kreonyx serve` et à `kreonyx web`.
...
Lorsque vous exécutez `kreonyx`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur kreonyx pour interagir avec kreonyx par programmation.
:::

Cette architecture permet à kreonyx de prendre en charge plusieurs clients et vous permet d'interagir avec kreonyx par programme.

Vous pouvez exécuter `kreonyx serve` pour démarrer un serveur autonome. Si vous avez le
kreonyx TUI en cours d'exécution, `kreonyx serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins KREONYX CLI [IDE](/docs/ide).
...
Le serveur kreonyx expose les API suivantes.
