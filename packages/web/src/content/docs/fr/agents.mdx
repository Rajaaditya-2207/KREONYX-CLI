...
Il existe deux types d'agents dans KREONYX CLI ; agents primaires et sous-agents.
...
KREONYX CLI est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
KREONYX CLI est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

KREONYX CLI est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `kreonyx.json` :
...
- Global : `~/.config/kreonyx/agents/`
- Par projet : `.kreonyx/agents/`

```markdown title="~/.config/kreonyx/agents/review.md"
---
...
Si aucune température n'est spécifiée, KREONYX CLI utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale KREONYX CLI et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration KREONYX CLI utilise le format `provider/model-id`. Par exemple, si vous utilisez [KREONYX CLI Zen](/docs/zen), vous utiliserez `kreonyx/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="kreonyx.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="kreonyx.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `kreonyx models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/anomalyco/kreonyx).
:::
...
```markdown title="~/.config/kreonyx/agents/docs-writer.md"
---
...
```markdown title="~/.config/kreonyx/agents/security-auditor.md"
---
