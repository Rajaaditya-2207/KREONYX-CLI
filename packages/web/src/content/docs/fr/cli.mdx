---
title: CLI
description: Options et commandes de la CLI KREONYX CLI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI KREONYX CLI démarre par défaut le [TUI](/docs/tui) lorsqu'il est exécuté sans aucun argument.

```bash
kreonyx
```

Mais elle accepte également les commandes documentées sur cette page. Cela vous permet d'interagir avec KREONYX CLI par programme.
...
Démarrez l’interface utilisateur du terminal KREONYX CLI.
...
La CLI KREONYX CLI dispose également des commandes suivantes.

---

### agent

Gérer les agents pour KREONYX CLI.
...
Attachez un terminal à un serveur backend KREONYX CLI déjà en cours d'exécution démarré via les commandes `serve` ou `web`.

```bash
kreonyx attach [url]
```

Cela permet d'utiliser le TUI avec un backend KREONYX CLI distant. Par exemple :
...
KREONYX CLI est alimenté par la liste des fournisseurs sur [Models.dev](https://models.dev), vous pouvez donc utiliser `kreonyx auth login` pour configurer les clés API pour tout fournisseur que vous souhaitez utiliser. Ceci est stocké dans `~/.local/share/kreonyx/auth.json`.

```bash
kreonyx auth login
```

Lorsque KREONYX CLI démarre, il charge les fournisseurs à partir du fichier d'informations d'identification, ainsi que les clés définies dans vos environnements ou un fichier `.env` dans votre projet.
...
Utilisez l'option `--refresh` pour mettre à jour la liste des modèles mis en cache. Ceci est utile lorsque de nouveaux modèles ont été ajoutés à un fournisseur et que vous souhaitez les voir dans KREONYX CLI.
...
Exécutez kreonyx en mode non interactif en transmettant directement un prompt.
...
Vous pouvez également vous connecter à une instance `kreonyx serve` en cours d'exécution pour éviter les temps de démarrage à froid du serveur MCP à chaque exécution :
...
| `--attach`   |       | Connectez-vous à un serveur kreonyx en cours d'exécution (par exemple, http://localhost:4096) |
...
Démarrez un serveur KREONYX CLI sans interface graphique pour un accès API. Consultez le [serveur docs](/docs/server) pour l'interface complète HTTP.

```bash
kreonyx serve
```

Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'KREONYX CLI sans l'interface TUI. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `kreonyx`).
...
Gérer les sessions KREONYX CLI.
...
Liste toutes les sessions KREONYX CLI.
...
Affichez les statistiques d'utilisation et de coût des jetons pour vos sessions KREONYX CLI.
...
Importez les données de session à partir d'un fichier JSON ou d'une URL de partage KREONYX CLI.

```bash
kreonyx import <file>
```

Vous pouvez importer à partir d'un fichier local ou d'une URL de partage KREONYX CLI.
...
Démarrez un serveur KREONYX CLI sans interface graphique avec une interface Web.

```bash
kreonyx web
```

Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à KREONYX CLI via une interface Web. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `kreonyx`).
...
Désinstallez KREONYX CLI et supprimez tous les fichiers associés.
...
Met à jour kreonyx vers la dernière version ou une version spécifique.
...
La CLI kreonyx prend les flags globaux suivants.
...
KREONYX CLI peut être configuré à l'aide de variables d'environnement.
...
| `OPENCODE_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `kreonyx`) |
