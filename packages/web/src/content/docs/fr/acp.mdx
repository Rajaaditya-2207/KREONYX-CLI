---
title: Support ACP
description: Utilisez KREONYX CLI dans n’importe quel éditeur compatible ACP.
---

KREONYX CLI prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser KREONYX CLI via ACP, configurez votre éditeur pour exécuter la commande `kreonyx acp`.

La commande démarre KREONYX CLI en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "KREONYX CLI": {
      "command": "/absolute/path/bin/kreonyx",
      "args": ["acp"]
    }
  }
}
...
Pour l'ouvrir, utilisez le nouvel agent « KREONYX CLI » dans le sélecteur d'agent AI Chat.
...
Pour utiliser KREONYX CLI comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser KREONYX CLI comme agent ACP pour le chat.
...
KREONYX CLI fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration KREONYX CLI
