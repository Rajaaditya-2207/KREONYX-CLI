...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez KREONYX CLI.
...
Voici plusieurs modèles qui fonctionnent bien avec KREONYX CLI, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration KREONYX CLI.

```json title="kreonyx.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [KREONYX CLI Zen](/docs/zen), vous utiliserez `kreonyx/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="kreonyx.jsonc" {7-12,19-24}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="kreonyx.jsonc" {6-21}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "kreonyx": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. KREONYX CLI est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

KREONYX CLI est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="kreonyx.jsonc" {7-18}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque KREONYX CLI démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration KREONYX CLI.
