...
Avec KREONYX CLI, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, KREONYX CLI utilise notre propre thème `kreonyx`.
...
KREONYX CLI est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que KREONYX CLI corresponde à l'apparence de leur terminal
...
KREONYX CLI prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/kreonyx/themes/*.json` ou `$XDG_CONFIG_HOME/kreonyx/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.kreonyx/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.kreonyx/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/kreonyx/themes
vim ~/.config/kreonyx/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .kreonyx/themes
vim .kreonyx/themes/my-theme.json
```
