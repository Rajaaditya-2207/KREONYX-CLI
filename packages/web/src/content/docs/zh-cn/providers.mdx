---
title: 提供商
description: 在 kreonyx 中使用任意 LLM 提供商。
---

import config from "../../../../config.mjs"
export const console = config.console

kreonyx 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev)，支持 **75+ LLM 提供商**，也支持运行本地模型。

要添加提供商，你需要：

1. 使用 `/connect` 命令添加提供商的 API 密钥。
2. 在 kreonyx 配置中设置该提供商。
...
你可以使用 kreonyx 配置中的 `provider` 部分自定义提供商配置。
...
KREONYX CLI Zen 是kreonyx团队提供的模型列表，这些模型已被
经测试和验证可与kreonyx良好配合。 [了解更多](/docs/zen)。

:::tip
如果您是新手，我们建议您从 KREONYX CLI Zen 开始。
:::
...
它的工作方式与 kreonyx 中的任何其他提供的程序相同，并且完全可以选择使用。
...
相当于 Amazon Bedrock 与 kreonyx 结合使用：
...
:::info
Using your Claude Pro/Max subscription in kreonyx is not officially supported by [Anthropic](https://anthropic.com).
:::
...
将Tokens存储在kreonyx身份验证存储中。
:::

##### 自托管 GitLab

:::note[合规笔记]
kreonyx 使用一个小模型来执行一些 AI 任务，例如生成会话标题。
情况下，其配置为使用 gpt-5-nano，由 Zen 托管。默认 kreonyx
...
相当于您的 GitHub Copilot 订阅与 kreonyx 一起使用：
...
Google Vertex AI 与 kreonyx 结合使用：
...
如果您发现Helicone的某些功能或模型未通过kreonyx自动配置，您始终可以自行配置。
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [kreonyx-helicone-session](https://github.com/H2Shami/kreonyx-helicone-session) plugin to automatically log each kreonyx conversation as a session in Helicone.
...
该外挂将 `Helicone-Session-Id` 和 `Helicone-Session-Name` 标头注入您的请求中。在 Helicone 的会话页面中，您将看到每个 kreonyx 对话都是单独的会话。
...
您可以通过使用本地模型来使用 LM Studio 配置kreonyx。
...
您可以使用 Ollama 配置 kreonyx 本地模型。

:::tip
Ollama can automatically configure itself for kreonyx. See the [Ollama integration docs](https://docs.ollama.com/integrations/kreonyx) for details.
:::
...
6. **重要**：在kreonyx中使用云模型之前，必须将模型信息拉取到本地：
...
KREONYX CLI Zen 是 kreonyx 团队提供的经过测试和验证的模型列表。 [了解更多](/docs/zen)。

1. 登录 **<a href={console}>KREONYX CLI Zen</a>** 并单击 **创建 API 密钥**。

2. 执行 `/connect` 命令并搜索 **KREONYX CLI Zen**。

   ```txt
   /connect
   ```

3. 输入您的 kreonyx API 密钥。
...
5. 您还可以使用kreonyx配置自定义它们。这是指定的示例
...
您还可以穿透 kreonyx 配置自定义模型。以下是指定提供商路由顺序的示例。
...
:::tip
您可以将任何 OpenAI 兼容的提供商与 kreonyx 一起使用。大多数 AI 提供商都提供 OpenAI 兼容 API。
:::
...
`limit` 栏位允许 kreonyx 了解您还剩下多少上下文。标准提供商会自动从 models.dev 中提取这些内容。
...
2. 对于自定义提供的程序，请检查 kreonyx 配置并：
   - 确保 `/connect` 命令中使用的提供商 ID 与 kreonyx 配置中的 ID 匹配。
