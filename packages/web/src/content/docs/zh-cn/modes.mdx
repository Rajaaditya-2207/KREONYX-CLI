...
:::caution
现在通过kreonyx配置中的`agent`选项配置模式。这
`mode` 选项现已废弃。 [了解更多](/docs/agents)。
:::

kreonyx 中的模式允许自定义不同的示例行为、工具和提示。

它具有两种内置模式：**构建**和**计划**。您可以定制
这些或通过 kreonyx 配置配置您自己的。
...
- `write` - 无法创建新文件
- `edit` - 无法修改现有文件，位于 `.kreonyx/plans/*.md` 的用于详细说明计划本身的文件另外
- `patch` - 无法应用补丁
- `bash` - 无法执行 shell 命令
...
如果未指定温度，kreonyx将使用特定于模型的默认值（大多数模型通常为0，Qwen模型为0.55）。
...
在`.kreonyx/modes/`中为项目特定模式创建模式文件，在`~/.config/kreonyx/modes/`中为全局模式创建模式文件：
