...
kreonyx 使用`permission` 配置来决定给定的操作是否应自动运行、提示您或被阻止。
...
使用 `external_directory` 允许工具调用启动 kreonyx 的工作目录之外的路径。这适用于任何采用路径作为输入的工具（例如 `read`、`edit`、`list`、`glob`、`grep` 和许多Z`bash` 命令）。
...
当 kreonyx 提示批准时，UI 会提供三种结果：

- `once` — 仅批准此请求
- `always` — 批准与建议模式匹配的未来请求（对于当前 kreonyx 会话的其余部分）
- `reject` — 拒绝请求
