---
title: ACP 支持
description: 在任何 ACP 兼容编辑器中使用 kreonyx。
---

kreonyx 支持 [Agent Client Protocol](https://agentclientprotocol.com) (ACP)，允许您直接在兼容的编辑器和 IDE 中使用它。
...
要通过 ACP 使用 kreonyx，须编辑器配置为运行 `kreonyx acp` 命令。

该命令将 kreonyx 作为 ACP 兼容的子进程启动，通过 stdio 通过 JSON-RPC 与您的编辑器进行通信。
...
要打开它，请在 AI Chat 代理选择器中使用新的 "kreonyx" 代理。
...
相当于 kreonyx 网关 [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim) 中的 ACP 代理，接下来将以下内容添加到 Neovim 配置中：
...
此配置将 CodeCompanion.nvim 设置为使用 kreonyx 作为聊天的 ACP 代理。
...
kreonyx 通过 ACP 的工作方式与在终端中的工作方式相同。支持所有功能：
