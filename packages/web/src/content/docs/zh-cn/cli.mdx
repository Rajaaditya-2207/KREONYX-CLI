---
title: CLI
description: kreonyx CLI 选项和命令。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

默认情况下，kreonyx CLI 在不带任何参数运行时启动 [TUI](/docs/tui)。

```bash
kreonyx
```

但它也接受本页记录的命令。这允许您以编程方式与 kreonyx 交互。
...
将终端附加到通过 `serve` 或 `web` 命令启动的已运行 kreonyx 后端服务器。

```bash
kreonyx attach [url]
```

这允许将 TUI 与远程 kreonyx 后端一起使用。例如：
...
kreonyx 由 [Models.dev](https://models.dev) 上的提供商列表支持，因此您可以使用 `kreonyx auth login` 来为您想要使用的任何提供商配置 API 密钥。它存储在 `~/.local/share/kreonyx/auth.json` 中。

```bash
kreonyx auth login
```

当 kreonyx 启动时，它会从凭据文件加载提供商。以及如果在您的环境或项目中的 `.env` 文件中定义了任何密钥。
...
使用 `--refresh` 标志来更新服务器的模型列表。当新模型已添加到提供商并且您希望在 kreonyx 中查看它们时，这非常有用。
...
您还可以附加到正在运行的 `kreonyx serve` 实例，以避免每次运行时 MCP 服务器冷启动时间：
...
| `--attach`   |      | 连接到正在运行的 kreonyx 服务器（例如，http://localhost:4096） |
...
启动无头 kreonyx 服务器以进行 API 访问。查看 [服务器文档](/docs/server) 以获取完整的 HTTP 接口。

```bash
kreonyx serve
```

这将启动一个 HTTP 服务器，该服务器提供对 kreonyx 功能的 API 访问，无需 TUI 界面。设置 `OPENCODE_SERVER_PASSWORD` 以启用 HTTP 基本身份验证（用户名默认为 `kreonyx`）。
...
显示 kreonyx 会话的 Token 使用情况和成本统计信息。
...
从 JSON 文件或 kreonyx 共享 URL 导入会话数据。

```bash
kreonyx import <file>
```

您可以从本地文件或 kreonyx 共享 URL 导入。
...
使用 Web 界面启动无头 kreonyx 服务器。

```bash
kreonyx web
```

这将启动 HTTP 服务器并打开 Web 浏览器以通过 Web 界面访问 kreonyx。设置 `OPENCODE_SERVER_PASSWORD` 以启用 HTTP 基本身份验证（用户名默认为 `kreonyx`）。
...
| `OPENCODE_SERVER_USERNAME`            | string  | 覆盖基本身份验证用户名（默认 `kreonyx`） |
