...
您可以使用“模型上下文协议”或MCP将外部工具添加到kreonyx。kreonyx支持本地和远程服务器。
...
您可以在`mcp`下的[kreonyx配置](https://opencode.ai/docs/config/)中定义MCP服务器。为每个MCP添加唯一的名称。当提示LLM时，您可以通过名称引用该MCP。
...
组织可以通过其 `.well-known/kreonyx` 端点提供默认的 MCP 服务器。这些服务器可能默认被禁用，允许用户选择他们需要的服务器。
...
kreonyx自动处理远程MCP服务器的OAuth身份验证。当服务器需要身份验证时，kreonyx将：
...
如果服务器需要身份验证，kreonyx 将在您第一次尝试使用它时提示您进行身份验证。如果没有，您可以使用 `timeout`[手动触发流量](#authenticating)。
...
`kreonyx mcp auth <server-name>` 命令将打开您的浏览器进行授权。授权后，kreonyx Tokens安全地存储在 `mcp auth` 中。

---

#### 禁用 OAuth

如果要禁用服务器的自动OAuth（例如，对于使用API密钥的服务器），则`~/.local/share/kreonyx/mcp-auth.json`设置为`oauth`：
...
您的 MCP 可以作为 kreonyx 中的工具以及内置工具使用。，您可以像任何其他工具一样通过 kreonyx 配置来管理它们。
...
这将打开一个浏览器窗口以完成 OAuth 流程并将 kreonyx 连接到您的 Sentry 账户。
