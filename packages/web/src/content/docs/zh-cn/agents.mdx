...
KREONYX CLI 附带两个内置的主代理：**Build** 和 **Plan**。
看看下面这些。
...
KREONYX CLI 附带两个内置子代理：**General** 和 **Explore**。我们将在下面看看这个。

---

## 内置

KREONYX CLI 附带两个内置主代理和两个内置子代理。
...
如果未指定温度，KREONYX CLI 将使用特定于模型的默认值；大多数模型通常为 0，Qwen 模型为 0.55。
...
该路径相对于文件所在位置的配置。因此，这适用于全局 KREONYX CLI 配置和项目特定配置。
...
KREONYX CLI 配置中的模型 ID 使用格式 `provider/model-id`。例如，如果您使用 [KREONYX CLI Zen](/docs/zen)，则您将使用 `kreonyx/gpt-5.1-codex` 来表示 GPT 5.1 Codex。
...
:::tip
您有想要分享的代理吗？ [提交 PR](https://github.com/anomalyco/kreonyx)。
:::
