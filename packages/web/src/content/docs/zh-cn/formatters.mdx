---
title: 格式化程序
description: kreonyx 使用特定于语言的格式化程序。
---

使用特定于语言的格式化程序编写或编辑文件后，kreonyx 会自动格式化文件。这可以确保生成的代码遵循项目的代码风格。

---

## 内置

kreonyx 附带了多个适用于流行语言和框架的内置格式化程序。下面是格式化程序、支持的文件扩展名以及所需的命令或配置选项的列表。
...
因此，如果您的项目的 `prettier` 中有 `package.json`，kreonyx 将自动使用它。

---

## 它是如何工作的

当 kreonyx 写入或编辑文件时，它：
...
您可以通过 kreonyx 配置中的 `formatter` 部分自定义格式化程序。
