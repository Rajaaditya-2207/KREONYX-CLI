---
title: 配置
description: 使用 kreonyx JSON 配置。
---

您可以使用 JSON 配置文件配置 kreonyx。

---

## 格式

kreonyx 支持 **JSON** 和 **JSONC**（带注释的 JSON）格式。
...
例如，如果您的全局配置设置 `theme: "kreonyx"` 和 `autoupdate: true`，并且您的项目配置设置 `model: "anthropic/claude-sonnet-4-5"`，则最终配置将包括所有三个设置。
...
1. **Remote config** (来自 `.well-known/kreonyx`) - 组织默认值
2. **Global config** (`~/.config/kreonyx/kreonyx.json`) - 用户首选项
3. **Custom config** (`OPENCODE_CONFIG` env var) - 自定义覆盖
4. **Project config** (项目中的 `kreonyx.json`) - 项目特定的设置
5. **`.kreonyx` 目录** - 代理、命令、插件
6. **Inline config** (`OPENCODE_CONFIG_CONTENT` env var) - 运行时覆盖

这意味着项目配置可以覆盖全局默认值，全局配置可以覆盖远程组织默认值。

:::note
`.kreonyx` 和 `~/.config/kreonyx` 目录对子目录使用 **复数名称**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。为了向后兼容，还支持单数名称（例如 `agent/`）。
:::
...
组织可以通过 `.well-known/kreonyx` 端点提供默认配置。当您向支持的提供商进行身份验证时，会自动获取该信息。
...
将全局 kreonyx 配置放在 `~/.config/kreonyx/kreonyx.json` 中。使用全局配置来实现用户范围的首选项，例如主题、提供商或按键绑定。
...
在项目根目录中添加 `kreonyx.json`。项目配置在标准配置文件中具有最高优先级 - 它覆盖全局配置和远程配置。

:::tip
将项目特定配置放在项目的根目录中。
:::

当 kreonyx 启动时，它会在当前目录中查找配置文件或向上遍历到最近的 Git 目录。
...
使用 `OPENCODE_CONFIG_DIR` 环境变量指定自定义配置目录。将在该目录中搜索代理、命令、模式和插件，就像标准 `.kreonyx` 目录一样，并且应该遵循相同的结构。
...
自定义目录在全局配置和 `.kreonyx` 目录加载后，因此 **可以覆盖** 它们的设置。
...
您可以通过 `kreonyx serve` 选项为 `kreonyx web` 和 `server` 命令配置服务器设置。
...
- `mdns` - 启用 mDNS 服务发现。这允许网络上的其他设备发现您的 kreonyx 服务器。
- `mdnsDomain` - mDNS 服务的自定义域名。默认为 `kreonyx.local`。对于在同一个网络上运行多个实例很有帮助。
...
您可以通过 `provider`、`model` 和 `small_model` 选项来配置要在 kreonyx 配置中使用的提供商和模型。
...
`small_model` 选项为标题生成等轻量级任务配置单独的模型。默认情况下，如果您的提供商可以提供更便宜的模型，kreonyx 会尝试使用更便宜的模型，否则它会退回到您的主模型。
...
您可以通过 kreonyx 配置中的 `theme` 选项配置要使用的主题。
...
您还可以使用 `~/.config/kreonyx/agents/` 或 `.kreonyx/agents/` 中的 markdown 文件定义代理。 [在这里了解更多](/docs/agents)。
...
默认代理必须是 Primary 代理（不是 Subagent）。这可以是内置代理（如 `"build"` 或 `"plan"`），也可以是您定义的 [Custom Agent](/docs/agents)。如果指定的代理不存在或者是子代理，kreonyx 将回退到 `"build"` 并发出警告。

此设置适用于所有界面：TUI、CLI (`kreonyx run`)、桌面应用程序和 GitHub Action。
...
您还可以使用 `~/.config/kreonyx/commands/` 或 `.kreonyx/commands/` 中的 Markdown 文件定义命令。 [在这里了解更多](/docs/commands)。
...
kreonyx 将在启动时自动下载任何新的更新。您可以使用 `autoupdate` 选项禁用此功能。
...
默认情况下，kreonyx **允许所有操作**，无需明确批准。您可以使用 `permission` 选项更改此设置。
...
[Plugins](/docs/plugins) 使用自定义工具、挂钩和集成扩展 kreonyx。

将插件文件放置在 `.kreonyx/plugins/` 或 `~/.config/kreonyx/plugins/` 中。您还可以通过 `plugin` 选项从 npm 加载插件。
...
当您想要限制 kreonyx 仅使用特定的提供商而不是逐一禁用它们时，这非常有用。
