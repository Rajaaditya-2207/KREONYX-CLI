---
title: 自定义工具
description: 创建 LLM 可以在 kreonyx 中调用的工具。
---

自定义工具是您创建的函数，LLM 可以在对话期间调用。它们与 kreonyx 的 [Built-in Tools](/docs/tools) 一起工作，例如 `read`、`write` 和 `bash`。
...
- 通过将它们放在项目的 `.kreonyx/tools/` 目录中来本地进行。
- 或者在全局范围内，将它们放在 `~/.config/kreonyx/tools/` 中。
