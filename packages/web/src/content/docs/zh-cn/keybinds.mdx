...
kreonyx 有一个按键绑定列表，您可以通过 kreonyx 配置进行自定义。
...
kreonyx 对大多数按键绑定使用 `leader` 键。这可以避免终端中的冲突。
...
kreonyx 桌面应用程序提示输入支持常见的 Readline/Emacs 风格的文本编辑快捷方式。这些是内置的，目前无法通过 `kreonyx.json` 进行配置。
