---
title: Сервер
description: Взаимодействуйте с сервером kreonyx через HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Команда `kreonyx serve` запускает автономный HTTP-сервер, который предоставляет конечную точку OpenAPI, которую может использовать клиент с открытым кодом.
...
| `--mdns-domain` | Custom domain name for mDNS service | `kreonyx.local` |
...
Установите `OPENCODE_SERVER_PASSWORD`, чтобы защитить сервер с помощью базовой аутентификации HTTP. Имя пользователя по умолчанию — `kreonyx` или установите `OPENCODE_SERVER_USERNAME`, чтобы переопределить его. Это относится как к `kreonyx serve`, так и к `kreonyx web`.
...
Когда вы запускаете `kreonyx`, он запускает TUI и сервер. Где находится TUI
...
:::tip
Используйте сервер kreonyx для программного взаимодействия с открытым кодом.
:::
...
Вы можете запустить `kreonyx serve`, чтобы запустить автономный сервер. Если у вас есть
TUI с открытым кодом запущен, `kreonyx serve` запустит новый сервер.
...
Конечную точку [`/tui`](#tui) можно использовать для управления TUI через сервер. Например, вы можете предварительно заполнить или запустить подсказку. Эта настройка используется плагинами kreonyx [IDE](/docs/ide).
...
Сервер kreonyx предоставляет следующие API.
