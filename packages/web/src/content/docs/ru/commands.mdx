...
Создайте `.kreonyx/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию kreonyx или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем kreonyx [config](/docs/config):
...
- Глобальный: `~/.config/kreonyx/commands/`
- Для каждого проекта: `.kreonyx/commands/`
...
kreonyx включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
