---
title: TUI
description: 使用 KREONYX CLI 終端機使用者介面。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KREONYX CLI 提供了一個互動式終端機介面或 TUI，供您與 LLM 一起處理專案。

執行 KREONYX CLI 會啟動當前目錄的 TUI。
...
使用 KREONYX CLI TUI 時，您可以輸入 `/` 後跟指令名稱來快速執行操作。例如：
...
將供應商添加到 KREONYX CLI。允許您從可用的供應商中進行選擇並添加其 API 金鑰。
...
您可以透過 KREONYX CLI 設定檔自定義 TUI 行為。
