...
您可以使用「模型上下文協定」或 MCP 將外部工具添加到 kreonyx。 kreonyx 支援本地和遠端伺服器。
...
您可以在 `mcp` 下的 [kreonyx 設定](https://opencode.ai/docs/config/) 中定義 MCP 伺服器。為每個 MCP 添加唯一的名稱。當提示 LLM 時，您可以透過名稱引用該 MCP。
...
組織可以透過其 `.well-known/kreonyx` 端點提供預設 MCP 伺服器。這些伺服器可能預設被禁用，允許使用者選擇他們需要的伺服器。
...
kreonyx 自動處理遠端 MCP 伺服器的 OAuth 身分驗證。當伺服器需要身分驗證時，kreonyx 將：
...
如果伺服器需要身分驗證，kreonyx 將在您第一次嘗試使用它時提示您進行身分驗證。如果沒有，您可以使用 `kreonyx mcp auth <server-name>` [手動觸發流程](#authenticating)。
...
`mcp auth` 指令將打開您的瀏覽器進行授權。授權後，kreonyx 會將權杖安全地儲存在 `~/.local/share/kreonyx/mcp-auth.json` 中。
...
您的 MCP 可作為 kreonyx 中的工具以及內建工具使用。因此，您可以像任何其他工具一樣透過 kreonyx 設定來管理它們。
...
這將打開一個瀏覽器視窗以完成 OAuth 流程並將 kreonyx 連接到您的 Sentry 帳號。
