---
title: 伺服器
description: 透過 HTTP 與 kreonyx 伺服器互動。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`kreonyx serve` 指令執行一個無介面 HTTP 伺服器，該伺服器公開 kreonyx 客戶端可以使用的 OpenAPI 端點。
...
| `--mdns-domain` | mDNS 服務的自定義網域名稱 | `kreonyx.local` |
| `--cors`        | 允許的其他瀏覽器來源      | `[]`             |
...
設定 `OPENCODE_SERVER_PASSWORD` 以使用 HTTP 基本身分驗證保護伺服器。使用者名稱預設為 `kreonyx`，或設定 `OPENCODE_SERVER_USERNAME` 來覆蓋它。這適用於 `kreonyx serve` 和 `kreonyx web`。
...
當您執行 `kreonyx` 時，它會啟動 TUI 和伺服器。 TUI 是
與伺服器對話的客戶端。伺服器公開 OpenAPI 3.1 規範
端點。該端點還用於生成 [軟體開發套件](/docs/sdk)。

:::tip
使用 kreonyx 伺服器以程式化方式與 kreonyx 進行互動。
:::

此架構讓 kreonyx 支援多個客戶端，並允許您以程式化方式與 kreonyx 進行互動。

您可以執行 `kreonyx serve` 來啟動獨立伺服器。如果您有
kreonyx TUI 執行，`kreonyx serve` 將啟動一個新伺服器。
...
[`/tui`](#tui) 端點可用於透過伺服器驅動 TUI。例如，您可以預填充或執行提示。此設定由 kreonyx [IDE](/docs/ide) 外掛使用。
