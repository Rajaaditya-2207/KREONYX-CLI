...
kreonyx 附帶兩個內建的主要代理：**Build** 和 **Plan**。我們將在下面看看這些。
...
kreonyx 附帶兩個內建子代理：**General** 和 **Explore**。我們將在下面看看這個。

---

## 內建

kreonyx 附帶兩個內建主代理和兩個內建子代理。
...
如果未指定溫度，kreonyx 將使用特定於模型的預設值；大多數模型通常為 0，Qwen 模型為 0.55。
...
該路徑是相對於設定檔所在位置的。因此，這適用於全域 kreonyx 設定和專案特定設定。
...
kreonyx 設定中的模型 ID 使用格式 `provider/model-id`。例如，如果您使用 [KREONYX CLI Zen](/docs/zen)，則您將使用 `kreonyx/gpt-5.1-codex` 來表示 GPT 5.1 Codex。
...
:::tip
您有想要分享的代理嗎？ [提交 PR](https://github.com/anomalyco/kreonyx)。
:::
