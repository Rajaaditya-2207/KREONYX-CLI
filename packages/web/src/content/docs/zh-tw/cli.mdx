---
title: 命令列介面
description: kreonyx CLI 選項和指令。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

預設情況下，KREONYX CLI CLI 在不帶任何參數執行時啟動 [TUI](/docs/tui)。

```bash
kreonyx
```

但它也接受本頁記錄的指令。這允許您以程式化方式與 KREONYX CLI 互動。
...
將終端機連接到透過 `serve` 或 `web` 指令啟動的已執行的 KREONYX CLI 後端伺服器。

```bash
kreonyx attach [url]
```

這允許將 TUI 與遠端 KREONYX CLI 後端一起使用。例如：
...
KREONYX CLI 由 [Models.dev](https://models.dev) 上的供應商列表提供支援，因此您可以使用 `kreonyx auth login` 為您想要使用的任何供應商設定 API 金鑰。它儲存在 `~/.local/share/kreonyx/auth.json` 中。

```bash
kreonyx auth login
```

當 KREONYX CLI 啟動時，它會從憑證檔案載入供應商。如果您的環境中定義了任何金鑰或專案中的 `.env` 檔案。
...
使用 `--refresh` 旗標來更新快取的模型列表。當新模型已新增到供應商並且您希望在 KREONYX CLI 中查看它們時，這非常有用。
...
您還可以附加到正在執行的 `kreonyx serve` 實例，以避免每次執行時 MCP 伺服器冷啟動時間：
...
| `--attach`   |      | 連接到正在執行的 kreonyx 伺服器（例如，http://localhost:4096） |
...
啟動無介面 kreonyx 伺服器以進行 API 存取。查看 [伺服器文件](/docs/server) 以獲取完整的 HTTP 介面。

```bash
kreonyx serve
```

這將啟動一個 HTTP 伺服器，該伺服器提供對 kreonyx 功能的 API 存取，而無需 TUI 介面。設定 `OPENCODE_SERVER_PASSWORD` 以啟用 HTTP 基本身分驗證（使用者名稱預設為 `kreonyx`）。
...
顯示 kreonyx 工作階段的 Tokens 使用情況和成本統計資訊。
...
從 JSON 檔案或 kreonyx 分享 URL 匯入工作階段數據。

```bash
kreonyx import <file>
```

您以從本地檔案或 kreonyx 分享 URL 匯入。
...
使用 Web 介面啟動無介面 kreonyx 伺服器。

```bash
kreonyx web
```

這將啟動 HTTP 伺服器並打開網頁瀏覽器以透過 Web 介面存取 kreonyx。設定 `OPENCODE_SERVER_PASSWORD` 以啟用 HTTP 基本身分驗證（使用者名稱預設為 `kreonyx`）。
...
| `OPENCODE_SERVER_USERNAME`            | string  | 覆蓋基本身分驗證使用者名稱（預設 `kreonyx`） |
