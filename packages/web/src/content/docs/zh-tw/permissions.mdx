...
kreonyx 使用 `permission` 配置來決定給定的操作是否應自動執行、提示您或被阻止。
...
使用 `external_directory` 允許工具呼叫存取啟動 kreonyx 的工作目錄之外的路徑。這適用於任何採用路徑作為輸入的工具（例如 `read`、`edit`、`list`、`glob`、`grep` 和許多 `bash` 指令）。
...
當 kreonyx 提示批准時，UI 會提供三種結果：

- `once` — 僅批准此請求
- `always` — 批准與建議模式匹配的未來請求（對於當前 kreonyx 工作階段的其餘部分）
- `reject` — 拒絕請求
