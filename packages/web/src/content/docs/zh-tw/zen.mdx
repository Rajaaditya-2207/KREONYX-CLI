...
KREONYX CLI Zen 是 kreonyx 團隊提供的經過測試和驗證的模型列表。

:::note
KREONYX CLI Zen 目前處於測試階段。
:::

Zen 的工作方式與 kreonyx 中的任何其他供應商一樣。您登入 KREONYX CLI Zen 並獲取
...
:::tip
我們測試了一組精選的與 kreonyx 配合良好的模型和供應商。
:::
...
KREONYX CLI Zen 是一個 AI 閘道，可讓您存取這些模型。

---

## 它是如何運作的

KREONYX CLI Zen 的工作方式與 kreonyx 中的任何其他供應商一樣。

1. 您登入 **<a href={console}>KREONYX CLI Zen</a>**，添加您的帳單
   詳細資訊，然後複製您的 API 金鑰。
2. 您在 TUI 中執行 `/connect` 指令，選擇 KREONYX CLI Zen，然後貼上您的 API 金鑰。
...
kreonyx 配置中的 [模型編號](/docs/config/#models)
使用格式 `kreonyx/<model-id>`。例如，對於 GPT 5.2 Codex，您將
在您的配置中使用 `kreonyx/gpt-5.2-codex`。
...
- GLM 4.7 免費版在 kreonyx 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Kimi K2.5 Free 在 kreonyx 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- MiniMax M2.1 免費版在 kreonyx 上限時提供。團隊正在利用這段時間收集回饋並改進模型。
- Big Pickle 是一個秘密模型，在 kreonyx 上限時免費。團隊正在利用這段時間收集回饋並改進模型。
