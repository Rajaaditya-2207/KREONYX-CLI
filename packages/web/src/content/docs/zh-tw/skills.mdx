...
代理技能讓 kreonyx 從您的儲存庫或主目錄中發現可重複使用的指令。
...
- 專案設定：`.kreonyx/skills/<name>/SKILL.md`
- 全域設定：`~/.config/kreonyx/skills/<name>/SKILL.md`
...
對於專案本地路徑，kreonyx 將從當前工作目錄開始，直到到達 git 工作樹。
它載入 `skills/*/SKILL.md` 中任何匹配的 `.kreonyx/` 以及一路上任何匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。

全域定義也從 `~/.config/kreonyx/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md` 和 `~/.agents/skills/*/SKILL.md` 載入。
...
像這樣建立 `.kreonyx/skills/git-release/SKILL.md`：
...
使用 `kreonyx.json` 中基於模式的權限控制代理可以存取哪些技能：
