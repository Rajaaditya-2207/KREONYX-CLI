---
title: 供應商
description: 使用 KREONYX CLI 中的任何 LLM 供應商。
---

import config from "../../../../config.mjs"
export const console = config.console

KREONYX CLI 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev) 來支援 **75+ LLM 供應商**，並且它支援執行本地模型。

要添加供應商，您需要：

1. 使用 `/connect` 指令添加供應商的 API 金鑰。
2. 在 KREONYX CLI 配置中配置供應商。
...
在 `~/.local/share/kreonyx/auth.json`。

---

### 配置

您可以在 KREONYX CLI 配置的 `provider` 部分自定義供應商。
...
KREONYX CLI Zen 是 KREONYX CLI 團隊提供的模型列表，這些模型已被
經過測試和驗證，可以與 KREONYX CLI 良好配合。 [了解更多](/docs/zen)。

:::tip
如果您是新手，我們建議您從 KREONYX CLI Zen 開始。
:::

1. 在 TUI 中執行 `/connect` 指令，選擇 kreonyx，然後前往 [opencode.ai/auth](https://opencode.ai/auth)。
...
它的工作方式與 KREONYX CLI 中的任何其他供應商一樣，並且完全可以選擇使用。
...
要將 Amazon Bedrock 與 KREONYX CLI 結合使用：
...
2. **使用以下方法之一配置身分驗證**：
   ...
   對於特定於專案或持久的配置，請使用 `kreonyx.json`：
...
:::info
[Anthropic](https://anthropic.com) 不正式支援在 KREONYX CLI 中使用您的 Claude Pro/Max 訂閱。
:::
...
2. 轉到 [Azure AI Foundry](https://ai.azure.com/) 並部署模型。
   ...
   :::note
   部署名稱必須與模型名稱匹配，KREONYX CLI 才能正常工作。
   :::
...
:::note
如果您不想將令牌儲存在 KREONYX CLI auth 儲存中，也可以指定 `GITLAB_TOKEN` 環境變數。
:::

##### 自託管 GitLab

:::note[合規性注意事項]
KREONYX CLI 使用小型模型來執行某些 AI 任務，例如生成工作階段標題。
預設情況下，它配置為使用 gpt-5-nano，由 Zen 託管。若要鎖定 KREONYX CLI
僅使用您自己的 GitLab 託管實例，請將以下內容添加到您的
`kreonyx.json` 檔案。還建議禁用工作階段分享。
...
更多文件請參見 [kreonyx-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) 主頁。
...
要將 GitHub Copilot 訂閱與 KREONYX CLI 結合使用：
...
要將 Google Vertex AI 與 KREONYX CLI 結合使用：
...
如果您發現 Helicone 的功能或模型未透過 KREONYX CLI 自動配置，您始終可以自行配置。
...
Helicone 的 [Sessions](https://docs.helicone.ai/features/sessions) 功能可讓您將相關的 LLM 請求分組在一起。使用 [kreonyx-helicone-session](https://github.com/H2Shami/kreonyx-helicone-session) 外掛自動將每個 KREONYX CLI 對話記錄為 Helicone 中的工作階段。
...
該外掛將 `Helicone-Session-Id` 和 `Helicone-Session-Name` 標頭注入您的請求中。在 Helicone 的 Sessions 頁面中，您將看到每個 KREONYX CLI 對話都列為單獨的工作階段。
...
您可以透過 [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server 公用程式配置 KREONYX CLI 以使用本地模型
...
您可以透過 LM Studio 配置 KREONYX CLI 以使用本地模型。
...
您可以透過 Ollama 配置 KREONYX CLI 以使用本地模型。

:::tip
Ollama 可以自動為 KREONYX CLI 配置自身。詳情請參閱 [Ollama integration docs](https://docs.ollama.com/integrations/kreonyx)。
:::
...
6. **Important**：在 KREONYX CLI 中使用雲端模型之前，您必須將模型資訊拉取到本地：
...
KREONYX CLI Zen 是 KREONYX CLI 團隊提供的經過測試和驗證的模型列表。 [了解更多](/docs/zen)。

1. 登入 **<a href={console}>KREONYX CLI Zen</a>** 並單擊 **Create API Key**。

2. 執行 `/connect` 指令並搜尋 **KREONYX CLI Zen**。

   ```txt
   /connect
   ```

3. 輸入您的 KREONYX CLI API 金鑰。
...
5. 您還可以透過 KREONYX CLI 配置自定義它們。這是指定供應商的範例
...
要將 [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) 與 KREONYX CLI 一起使用：
...
您還可以透過 KREONYX CLI 配置自定義模型。以下是指定供應商路由順序的範例。
...
:::tip
您可以將任何與 OpenAI 相容的供應商與 KREONYX CLI 結合使用。大多數現代 AI 供應商都提供與 OpenAI 相容的 API。
:::
...
`limit` 欄位允許 KREONYX CLI 了解您還剩下多少上下文。標準供應商會自動從 models.dev 中提取這些內容。
...
1. **檢查 auth 設定**：執行 `kreonyx auth list` 以查看憑證是否
   供應商的配置已添加到您的配置中。

   這不適用於 Amazon Bedrock 等依賴環境變數進行身分驗證的供應商。

2. 對於自定義供應商，請檢查 KREONYX CLI 配置並：
   - 確保 `/connect` 指令中使用的供應商 ID 與 KREONYX CLI 配置中的 ID 匹配。
