...
預設情況下會預先載入大多數流行的供應商。如果您已透過 `/connect` 指令添加了供應商的憑證，那麼它們將在您啟動 KREONYX CLI 時可用。
...
以下是與 KREONYX CLI 配合良好的幾個模型，排名不分先後。 （這不是詳盡的列表，也不一定是最新的）：
...
這裡完整的 ID 是 `provider_id/model_id`。例如，如果您使用 [KREONYX CLI Zen](/docs/zen)，則您將使用 `kreonyx/gpt-5.1-codex` 來表示 GPT 5.1 Codex。
...
許多模型支援具有不同配置的多種變體。KREONYX CLI 附帶了流行供應商的內建預設變體。

### 內建變體

KREONYX CLI 附帶了許多供應商的預設變體：
...
當 KREONYX CLI 啟動時，它會按以下優先順序檢查模型：

1. `--model` 或 `-m` 命令列旗標。格式與設定檔中的相同：`provider_id/model_id`。

2. KREONYX CLI 配置中的模型列表。
