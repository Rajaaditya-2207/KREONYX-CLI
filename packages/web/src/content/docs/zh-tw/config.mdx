---
title: 設定
description: 使用 kreonyx JSON 設定。
---

您可以使用 JSON 設定檔設定 kreonyx。

---

## 格式

kreonyx 支援 **JSON** 和 **JSONC**（帶註解的 JSON）格式。
...
例如，如果您的全域設定設定 `theme: "kreonyx"` 和 `autoupdate: true`，並且您的專案設定設定 `model: "anthropic/claude-sonnet-4-5"`，則最終設定將包括所有三個設定。
...
1. **遠端設定**（來自 `.well-known/kreonyx`）- 組織預設值
2. **全域設定** (`~/.config/kreonyx/kreonyx.json`) - 使用者偏好設定
3. **自定義設定** (`OPENCODE_CONFIG` env var) - 自定義覆寫
4. **專案設定**（專案中的 `kreonyx.json`）- 專案特定的設定
5. **`.kreonyx` 目錄** - 代理、指令、外掛
6. **內聯設定** (`OPENCODE_CONFIG_CONTENT` env var) - 執行時覆寫

這意味著專案設定可以覆寫全域預設值，全域設定可以覆寫遠端組織預設值。

:::note
`.kreonyx` 和 `~/.config/kreonyx` 目錄對子目錄使用**複數名稱**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。為了向後相容，也支援單數名稱（例如 `agent/`）。
:::
...
組織可以透過 `.well-known/kreonyx` 端點提供預設設定。當您向支援它的供應商進行身分驗證時，會自動取得該資訊。
...
將全域 kreonyx 設定放在 `~/.config/kreonyx/kreonyx.json` 中。使用全域設定來實現使用者範圍的偏好設定，例如主題、供應商或按鍵綁定。
...
在專案根目錄中新增 `kreonyx.json`。專案設定在標準設定檔中具有最高優先級 - 它覆寫全域設定和遠端設定。

:::tip
將專案特定設定放在專案的根目錄中。
:::

當 kreonyx 啟動時，它會在當前目錄中尋找設定檔或遍歷到最近的 Git 目錄。
...
使用 `OPENCODE_CONFIG_DIR` 環境變數指定自定義設定目錄。將在該目錄中搜尋代理、指令、模式和外掛，就像標準 `.kreonyx` 目錄一樣，並且應該遵循相同的結構。
...
自定義目錄在全域設定和 `.kreonyx` 目錄之後載入，因此它**可以覆寫**它們的設定。
...
您可以透過 `kreonyx serve` 選項為 `kreonyx web` 和 `server` 指令設定伺服器設定。
...
- `mdns` - 啟用 mDNS 服務探索。這允許網路上的其他設備發現您的 kreonyx 伺服器。
- `mdnsDomain` - mDNS 服務的自定義網域名稱。預設為 `kreonyx.local`。對於在同一網路上執行多個實例很有用。
...
您可以透過 `provider`、`model` 和 `small_model` 選項來設定要在 kreonyx 設定中使用的供應商和模型。
...
`small_model` 選項為標題生成等輕量級任務設定單獨的模型。預設情況下，如果您的供應商可以提供更便宜的模型，kreonyx 會嘗試使用更便宜的模型，否則它會退回到您的主模型。
...
您可以透過 `theme` 選項在 kreonyx 設定中設定要使用的主題。
...
您也可以使用 `~/.config/kreonyx/agents/` 或 `.kreonyx/agents/` 中的 markdown 檔案定義代理。 [在這裡了解更多](/docs/agents)。
...
預設代理必須是主代理（而不是子代理）。這可以是內建代理，例如 `"build"` 或 `"plan"`，或者您定義的 [自定義代理](/docs/agents)。如果指定的代理不存在或者是子代理，kreonyx 將退回到 `"build"` 並發出警告。

此設定適用於所有介面：TUI、CLI (`kreonyx run`)、桌面應用程式和 GitHub Action。
...
您也可以使用 `~/.config/kreonyx/commands/` 或 `.kreonyx/commands/` 中的 Markdown 檔案定義指令。 [在這裡了解更多](/docs/commands)。
...
kreonyx 將在啟動時自動下載任何新的更新。您可以使用 `autoupdate` 選項禁用此功能。
...
預設情況下，kreonyx **允許所有操作**，無需明確批准。您可以使用 `permission` 選項更改此設定。
...
[外掛](/docs/plugins) 使用自定義工具、掛鉤和整合擴展 kreonyx。

將外掛檔案放置在 `.kreonyx/plugins/` 或 `~/.config/kreonyx/plugins/` 中。您也可以透過 `plugin` 選項從 npm 載入外掛。
...
當您想要限制 kreonyx 僅使用特定的供應商而不是一一禁用它們時，這非常有用。
