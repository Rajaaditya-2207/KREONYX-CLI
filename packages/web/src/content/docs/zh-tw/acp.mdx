---
title: ACP 支援
description: 在任何 ACP 相容編輯器中使用 KREONYX CLI。
---

KREONYX CLI 支援 [Agent Client Protocol](https://agentclientprotocol.com) 或 (ACP)，允許您直接在相容的編輯器和 IDE 中使用它。
...
要透過 ACP 使用 KREONYX CLI，請將編輯器設定為執行 `kreonyx acp` 指令。

該指令將 KREONYX CLI 作為 ACP 相容的子程序啟動，透過 stdio 透過 JSON-RPC 與您的編輯器進行通訊。
...
要打開它，請在 AI Chat 代理選擇器中使用新的「kreonyx」代理。
...
要將 KREONYX CLI 用作 [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim) 中的 ACP 代理，請將以下內容新增到 Neovim 設定中：
...
此設定將 CodeCompanion 設定為使用 KREONYX CLI 作為聊天的 ACP 代理。
...
KREONYX CLI 透過 ACP 的工作方式與在終端機中的工作方式相同。支援所有功能：
