---
title: 格式化程式
description: kreonyx 使用特定於語言的格式化程式。
---

使用特定於語言的格式化程式編寫或編輯檔案後，kreonyx 會自動格式化檔案。這可確保生成的程式碼遵循專案的程式碼風格。

---

## 內建

kreonyx 附帶了多個適用於流行語言和框架的內建格式化程式。下面是格式化程式、支援的檔案副檔名以及所需的指令或設定選項的列表。
...
因此，如果您的專案的 `package.json` 中有 `prettier`，kreonyx 將自動使用它。

---

## 它是如何運作的

當 kreonyx 寫入或編輯檔案時，它：
...
您可以透過 kreonyx 設定中的 `formatter` 部分自定義格式化程式。
