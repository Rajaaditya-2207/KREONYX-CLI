...
:::caution
現在透過 kreonyx 設定中的 `agent` 選項配置模式。這
`mode` 選項現已棄用。 [了解更多](/docs/agents)。
:::

kreonyx 中的模式允許您自定義不同使用案例的行為、工具和提示。

它具有兩種內建模式：**建置 (Build)**和**計畫 (Plan)**。您可以自定義
這些或透過 kreonyx 設定配置您自己的。
...
- `write` - 無法建立新檔案
- `edit` - 無法修改現有檔案，位於 `.kreonyx/plans/*.md` 的用於詳細說明計畫本身的檔案除外
- `patch` - 無法套用 Patch
- `bash` - 無法執行 shell 指令
...
在 `.kreonyx/modes/` 中為專案特定模式建立模式檔案，在 `~/.config/kreonyx/modes/` 中為全域模式建立模式檔案：
