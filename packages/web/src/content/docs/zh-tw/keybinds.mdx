...
kreonyx 有一個按鍵綁定列表，您可以透過 kreonyx 設定進行自定義。
...
kreonyx 對大多數按鍵綁定使用 `leader` 鍵。這可以避免終端機中的衝突。
...
kreonyx 桌面應用程式提示輸入支援常見的 Readline/Emacs 風格的文字編輯快速鍵。這些是內建的，目前無法透過 `kreonyx.json` 進行設定。
