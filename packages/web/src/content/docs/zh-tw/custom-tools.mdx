---
title: 自定義工具
description: 建立 LLM 可以在 kreonyx 中呼叫的工具。
---

自定義工具是您建立的函式，LLM 可以在對話期間呼叫。它們與 kreonyx 的 [內建工具](/docs/tools) 一起工作，例如 `read`、`write` 和 `bash`。
...
- 透過將它們放在專案的 `.kreonyx/tools/` 目錄中來本地進行。
- 或者在全域範圍內，將它們放置在 `~/.config/kreonyx/tools/` 中。
