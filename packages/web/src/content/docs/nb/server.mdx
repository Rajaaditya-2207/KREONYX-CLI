---
title: Tjener
description: Samhandle med KREONYX CLI-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `kreonyx serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en KREONYX CLI-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `kreonyx.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `kreonyx` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `kreonyx serve` og `kreonyx web`.
...
Når du kjører `kreonyx` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk KREONYX CLI-serveren til å samhandle med KREONYX CLI programmatisk.
:::

Denne arkitekturen lar KREONYX CLI støtte flere klienter og lar deg samhandle med KREONYX CLI programmatisk.

Du kan kjøre `kreonyx serve` for å starte en frittstående server. Hvis du har
KREONYX CLI TUI kjører, vil `kreonyx serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av KREONYX CLI [IDE](/docs/ide) plugins.
...
kreonyx-serveren viser følgende APIer.
