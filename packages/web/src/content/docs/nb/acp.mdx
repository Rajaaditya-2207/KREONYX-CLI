---
title: ACP-støtte
description: Bruk kreonyx i en hvilken som helst ACP-kompatibel editor.
---

KREONYX CLI støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke KREONYX CLI via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `kreonyx acp`.

Kommandoen starter KREONYX CLI som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'KREONYX CLI'-agenten i AI Chat-agentvelgeren.
...
For å bruke KREONYX CLI som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke KREONYX CLI som ACP-agent for chat.
...
KREONYX CLI fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i KREONYX CLI-konfigurasjonen
