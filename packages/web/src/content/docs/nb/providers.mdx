---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i KREONYX CLI.
---

import config from "../../../../config.mjs"
export const console = config.console

KREONYX CLI bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i KREONYX CLI-konfigurasjonen.
...
i `~/.local/share/kreonyx/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i KREONYX CLI
konfig.
...
KREONYX CLI Zen er en liste over modeller levert av KREONYX CLI-teamet som har vært
testet og verifisert for å fungere godt med KREONYX CLI. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med KREONYX CLI Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg kreonyx og gå til [opencode.ai/auth](https://opencode.ai/auth).
...
Det fungerer som alle andre leverandører i KREONYX CLI og er helt valgfritt å bruke.
...
Slik bruker du Amazon Bedrock med KREONYX CLI:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører KREONYX CLI:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `kreonyx.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i kreonyx støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at KREONYX CLI skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom KREONYX CLI-konfigurasjonen.
...
for å lagre token i KREONYX CLI auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
KREONYX CLI bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse KREONYX CLI
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`kreonyx.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [kreonyx-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `kreonyx.json`:
...
Slik bruker du GitHub Copilot-abonnementet med KREONYX CLI:
...
Slik bruker du Google Vertex AI med KREONYX CLI:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører KREONYX CLI.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom KREONYX CLI, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [kreonyx-helicone-session](https://github.com/H2Shami/kreonyx-helicone-session) plugin for automatisk å logge hver KREONYX CLI-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver KREONYX CLI-samtale oppført som en separat økt.
...
Du kan konfigurere KREONYX CLI for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere KREONYX CLI for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere KREONYX CLI for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for KREONYX CLI. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/kreonyx) for detaljer.
:::
...
Slik bruker du Ollama Cloud med KREONYX CLI:
...
3. Kopier API-nøkkelen for bruk i KREONYX CLI.
...
6. **Viktig**: Før du bruker skymodeller i KREONYX CLI, må du hente modellinformasjonen lokalt:
...
KREONYX CLI Zen er en liste over testede og verifiserte modeller levert av KREONYX CLI-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>KREONYX CLI Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **KREONYX CLI Zen**.

   ```txt
   /connect
   ```

3. Skriv inn KREONYX CLI API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom KREONYX CLI-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom KREONYX CLI-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med KREONYX CLI:
...
Du kan også tilpasse modeller gjennom KREONYX CLI-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med KREONYX CLI. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `kreonyx.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar KREONYX CLI forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `kreonyx auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk KREONYX CLI-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i KREONYX CLI-konfigurasjonen.
