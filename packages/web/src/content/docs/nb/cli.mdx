---
title: CLI
description: KREONYX CLI CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KREONYX CLI CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
kreonyx
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med KREONYX CLI programmatisk.
...
Start KREONYX CLI-terminalbrukergrensesnittet.
...
Administrer agenter for KREONYX CLI.
...
Koble en terminal til en allerede kjørende KREONYX CLI-backend-server startet via `serve` eller `web` kommandoer.

```bash
kreonyx attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern KREONYX CLI-backend. For eksempel:
...
KREONYX CLI drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `kreonyx auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/kreonyx/auth.json`.

```bash
kreonyx auth login
```

Når KREONYX CLI starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i KREONYX CLI.
...
Kjør KREONYX CLI i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `kreonyx serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| `--attach`   |      | Koble til en kjørende KREONYX CLI-server (f.eks. http://localhost:4096)     |
...
Start en headless KREONYX CLI-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
kreonyx serve
```

Dette starter en HTTP-server som gir API tilgang til KREONYX CLI-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `kreonyx`).
...
List opp alle KREONYX CLI-økter.
...
Vis tokenbruk og kostnadsstatistikk for KREONYX CLI-øktene dine.
...
Importer øktdata fra en JSON-fil eller en KREONYX CLI share-URL.
...
Start en headless KREONYX CLI-server med et webgrensesnitt.

```bash
kreonyx web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til KREONYX CLI via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `kreonyx`).
...
Avinstaller KREONYX CLI og fjern alle relaterte filer.
...
Oppdaterer KREONYX CLI til siste versjon eller en spesifikk versjon.
...
KREONYX CLI CLI bruker følgende globale flagg.
...
KREONYX CLI kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `kreonyx`) |
