...
Det er to typer agenter i KREONYX CLI; primære agenter og underagenter.
...
KREONYX CLI kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
KREONYX CLI kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

KREONYX CLI kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `kreonyx.json`:
...
- Globalt: `~/.config/kreonyx/agents/`
- Per prosjekt: `.kreonyx/agents/`
...
Hvis ingen temperatur er spesifisert, bruker KREONYX CLI modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale KREONYX CLI-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i KREONYX CLI-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [KREONYX CLI Zen](/docs/zen), vil du bruke `kreonyx/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `kreonyx models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/kreonyx).
:::
