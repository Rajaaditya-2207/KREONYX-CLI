...
Du kan legge til eksterne verktøy i KREONYX CLI ved å bruke _Model Context Protocol_ eller MCP. KREONYX CLI støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [KREONYX CLI Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="kreonyx.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/kreonyx`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
KREONYX CLI håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil KREONYX CLI:
...
Hvis serveren krever autentisering, vil KREONYX CLI be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `kreonyx mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil KREONYX CLI lagre tokens sikkert i `~/.local/share/kreonyx/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i KREONYX CLI, sammen med innebygde verktøy. Så du kan administrere dem gjennom KREONYX CLI-konfigurasjonen som alle andre verktøy.
...
```json title="kreonyx.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble KREONYX CLI til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver KREONYX CLI-samtale oppført som en separat økt.
