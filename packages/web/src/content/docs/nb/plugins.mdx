...
description: Skriv dine egne programtillegg for å utvide kreonyx.
---

Plugins lar deg utvide KREONYX CLI ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til KREONYX CLI.
...
- `.kreonyx/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/kreonyx/node_modules/`.
...
1. Global konfigurasjon (`~/.config/kreonyx/kreonyx.json`)
2. Prosjektkonfigurasjon (`kreonyx.json`)
3. Global plugin-katalog (`~/.config/kreonyx/plugins/`)
4. Prosjektpluginkatalog (`.kreonyx/plugins/`)
...
KREONYX CLI kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En KREONYX CLI SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide KREONYX CLI.
...
:::note
Hvis du bruker KREONYX CLI-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre KREONYX CLI fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til KREONYX CLI:
...
`tool`-hjelperen lager et tilpasset verktøy som KREONYX CLI kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for KREONYX CLI sammen med innebygde verktøy.
