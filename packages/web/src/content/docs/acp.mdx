---
title: ACP Support
description: Use KREONYX CLI in any ACP-compatible editor.
---

KREONYX CLI supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use KREONYX CLI via ACP, configure your editor to run the `kreonyx acp` command.

The command starts KREONYX CLI as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'KREONYX CLI' agent in the AI Chat agent selector.
...
To use KREONYX CLI as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use KREONYX CLI as the ACP agent for chat.
...
KREONYX CLI works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your KREONYX CLI config
