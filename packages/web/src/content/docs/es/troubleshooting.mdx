...
Para depurar problemas con KREONYX CLI, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/kreonyx/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\kreonyx\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `kreonyx --log-level DEBUG`.

---

## Almacenamiento

kreonyx almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/kreonyx/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\kreonyx`
...
KREONYX CLI Desktop ejecuta un servidor KREONYX CLI local (el sidecar `kreonyx-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `KREONYX CLI` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/kreonyx/kreonyx.jsonc` (o `~/.config/kreonyx/kreonyx.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/kreonyx/kreonyx.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\kreonyx\kreonyx.jsonc`
...
KREONYX CLI también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/kreonyx/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\kreonyx\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que KREONYX CLI pueda reconstruirlo.

1. Salga de KREONYX CLI Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/kreonyx`
- **Linux**: eliminar `~/.cache/kreonyx` (o ejecutar `rm -rf ~/.cache/kreonyx`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\kreonyx`

3. Reinicie el escritorio KREONYX CLI.

---

### Solucionar problemas de conexión del servidor

KREONYX CLI Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `kreonyx.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio KREONYX CLI requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de KREONYX CLI.

---

### Notificaciones que no se muestran

KREONYX CLI Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para KREONYX CLI en la configuración de su sistema operativo, y
...
1. Salga del escritorio KREONYX CLI.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio KREONYX CLI):

- `kreonyx.settings.dat` (URL del servidor predeterminado de escritorio)
- `kreonyx.global.dat` y `kreonyx.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con KREONYX CLI:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/kreonyx/issues**](https://github.com/anomalyco/kreonyx/issues)
...
3. Asegúrese de tener la última versión con `kreonyx upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `kreonyx models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/kreonyx
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\kreonyx`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. kreonyx instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\kreonyx`

2. Reinicie kreonyx para reinstalar los paquetes del proveedor más recientes.

Esto obligará a kreonyx a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
kreonyx detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
