...
Las habilidades del agente permiten a KREONYX CLI descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
KREONYX CLI busca estas ubicaciones:

- Configuración del proyecto: `.kreonyx/skills/<name>/SKILL.md`
- Configuración global: `~/.config/kreonyx/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, KREONYX CLI sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.kreonyx/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/kreonyx/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.kreonyx/skills/git-release/SKILL.md` así:
...
KREONYX CLI enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `kreonyx.json`:
...
**Para agentes integrados** (en `kreonyx.json`):
