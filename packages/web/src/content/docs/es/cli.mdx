---
title: CLI
description: Opciones y comandos de la CLI de KREONYX CLI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de KREONYX CLI de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
kreonyx
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con KREONYX CLI mediante programación.
...
Inicie la interfaz de usuario del terminal KREONYX CLI.
...
La CLI de KREONYX CLI también tiene los siguientes comandos.

---

### agent

Administrar agentes para KREONYX CLI.
...
Conecte una terminal a un servidor backend KREONYX CLI que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
kreonyx attach [url]
```

Esto permite usar TUI con un backend remoto KREONYX CLI. Por ejemplo:
...
KREONYX CLI funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `kreonyx auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/kreonyx/auth.json`.

```bash
kreonyx auth login
```

Cuando se inicia KREONYX CLI, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en KREONYX CLI.
...
Ejecute kreonyx en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `kreonyx serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| `--attach`   |       | Adjuntar a un servidor kreonyx en ejecución (por ejemplo, http://localhost:4096)   |
...
Inicie un servidor KREONYX CLI sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
kreonyx serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad kreonyx sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `kreonyx`).
...
Administrar KREONYX CLI sesiones.
...
Enumere todas las sesiones KREONYX CLI.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones KREONYX CLI.
...
Importe datos de sesión desde un archivo JSON o una URL compartida KREONYX CLI.

```bash
kreonyx import <file>
```

Puede importar desde un archivo local o una URL compartida KREONYX CLI.
...
Inicie un servidor KREONYX CLI sin cabeza con una interfaz web.

```bash
kreonyx web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a KREONYX CLI a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `kreonyx`).
...
Desinstale KREONYX CLI y elimine todos los archivos relacionados.
...
Actualiza kreonyx a la última versión o a una versión específica.
...
La CLI de KREONYX CLI toma las siguientes banderas globales.
...
KREONYX CLI se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `kreonyx`) |
