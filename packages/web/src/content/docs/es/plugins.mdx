---
title: Complementos
description: Escriba sus propios complementos para extender KREONYX CLI.
---

Los complementos le permiten extender KREONYX CLI al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de KREONYX CLI.
...
- `.kreonyx/plugins/` - Complementos a nivel de proyecto
- `~/.config/kreonyx/plugins/` - Complementos globales
...
```json title="kreonyx.json"
{
  ...
  "plugin": ["kreonyx-helicone-session", "kreonyx-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/kreonyx/node_modules/`.
...
1. Configuración global (`~/.config/kreonyx/kreonyx.json`)
2. Configuración del proyecto (`kreonyx.json`)
3. Directorio global de complementos (`~/.config/kreonyx/plugins/`)
4. Directorio de complementos del proyecto (`.kreonyx/plugins/`)
...
```json title=".kreonyx/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
KREONYX CLI ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".kreonyx/plugins/my-plugin.ts"
...
```js title=".kreonyx/plugins/example.js"
...
- `client`: Un cliente SDK kreonyx para interactuar con la IA.
...
import type { Plugin } from "@kreonyx/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar kreonyx.
...
```js title=".kreonyx/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "kreonyx"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio KREONYX CLI, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que kreonyx lea archivos `.env`:

```javascript title=".kreonyx/plugins/env-protection.js"
...
```javascript title=".kreonyx/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a kreonyx:

```ts title=".kreonyx/plugins/custom-tools.ts"
import { type Plugin, tool } from "@kreonyx/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que kreonyx puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para kreonyx junto con las herramientas integradas.
...
```ts title=".kreonyx/plugins/compaction.ts"
import type { Plugin } from "@kreonyx/plugin"
...
```ts title=".kreonyx/plugins/custom-compaction.ts"
import type { Plugin } from "@kreonyx/plugin"
