...
:::caution
Los modos ahora se configuran a través de la opción `agent` en la configuración kreonyx. El
La opción `mode` ahora está en desuso. [Más información](/docs/agents).
:::

Los modos en kreonyx le permiten personalizar el comportamiento, las herramientas y las indicaciones para diferentes casos de uso.

Viene con dos modos integrados: **construir** y **planificar**. Puedes personalizar
estos o configure el suyo propio a través de la configuración kreonyx.
...
kreonyx viene con dos modos integrados.
...
- `edit` - No se pueden modificar archivos existentes, excepto los archivos ubicados en `.kreonyx/plans/*.md` para detallar el plan en sí.
...
Configure los modos en su archivo de configuración `kreonyx.json`:
...
Si no se especifica ninguna temperatura, kreonyx utiliza valores predeterminados específicos del modelo (normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen).
...
tanto la configuración global kreonyx como la configuración específica del proyecto.
...
Cree archivos de modo en `.kreonyx/modes/` para modos específicos del proyecto o `~/.config/kreonyx/modes/` para modos globales:
