---
title: Proveedores
description: Usando cualquier proveedor LLM en KREONYX CLI.
---

import config from "../../../../config.mjs"
export const console = config.console

KREONYX CLI usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración KREONYX CLI.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu KREONYX CLI
configuración.
...
KREONYX CLI Zen es una lista de modelos proporcionados por el equipo KREONYX CLI que han sido
probado y verificado para funcionar bien con KREONYX CLI. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con KREONYX CLI Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione kreonyx y diríjase a [opencode.ai/auth](https://opencode.ai/auth).
...
Funciona como cualquier otro proveedor en KREONYX CLI y su uso es completamente opcional.
...
Para usar Amazon Bedrock con KREONYX CLI:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta kreonyx:
   ...
   Para una configuración persistente o específica del proyecto, utilice `kreonyx.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en KREONYX CLI no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que kreonyx funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración kreonyx.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `KREONYX CLI`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación kreonyx.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
KREONYX CLI utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear KREONYX CLI
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `kreonyx.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [kreonyx-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configuración

Personalizar a través de `kreonyx.json`:
...
Para utilizar su suscripción GitHub Copilot con kreonyx:
...
Para utilizar Google Vertex AI con KREONYX CLI:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta kreonyx.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de kreonyx, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [kreonyx-helicone-session](https://github.com/H2Shami/kreonyx-helicone-session) para registrar automáticamente cada conversación KREONYX CLI como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación KREONYX CLI enumerada como una sesión separada.
...
Puede configurar kreonyx para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar kreonyx para usar modelos locales a través de LM Studio.
...
Puedes configurar kreonyx para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para KREONYX CLI. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/kreonyx) para obtener más detalles.
:::
...
Para usar Ollama Cloud con KREONYX CLI:
...
3. Copie la clave API para usarla en KREONYX CLI.
...
6. **Importante**: Antes de usar modelos en la nube en KREONYX CLI, debe obtener la información del modelo localmente:
...
KREONYX CLI Zen es una lista de modelos probados y verificados proporcionada por el equipo KREONYX CLI. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>KREONYX CLI Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **KREONYX CLI Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave KREONYX CLI API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración kreonyx.
...
5. También puedes personalizarlos a través de tu configuración kreonyx. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con KREONYX CLI:
...
También puede personalizar modelos a través de su configuración kreonyx. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con kreonyx. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `kreonyx.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a KREONYX CLI comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `kreonyx auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración kreonyx y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración kreonyx.
