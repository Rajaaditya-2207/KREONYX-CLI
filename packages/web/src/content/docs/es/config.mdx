---
title: Configuración
description: Usando la configuración KREONYX CLI JSON.
---

Puede configurar KREONYX CLI usando un archivo de configuración JSON.

---

## Formato

KREONYX CLI admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
Por ejemplo, si su configuración global establece `theme: "kreonyx"` y `autoupdate: true`, y la configuración de su proyecto establece `model: "anthropic/claude-sonnet-4-5"`, la configuración final incluirá las tres configuraciones.
...
1. **Configuración remota** (de `.well-known/kreonyx`): valores predeterminados de la organización
2. **Configuración global** (`~/.config/kreonyx/kreonyx.json`) - preferencias del usuario
3. **Configuración personalizada** (`OPENCODE_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`kreonyx.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.kreonyx`** - agentes, comandos, complementos
...
:::note
Los directorios `.kreonyx` y `~/.config/kreonyx` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Las organizaciones pueden proporcionar una configuración predeterminada a través del punto final `.well-known/kreonyx`. Esto se obtiene automáticamente cuando se autentica con un proveedor que lo admita.
...
Coloque su configuración global KREONYX CLI en `~/.config/kreonyx/kreonyx.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `kreonyx.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.

:::tip
Coloque la configuración específica del proyecto en la raíz de su proyecto.
:::

Cuando se inicia KREONYX CLI, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
modos y complementos como el directorio estándar `.kreonyx`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.kreonyx`, por lo que **puede anular** su configuración.
...
Puede configurar los ajustes del servidor para los comandos `kreonyx serve` y `kreonyx web` a través de la opción `server`.
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor KREONYX CLI.
- `mdnsDomain` - Nombre de dominio personalizado para el servicio mDNS. El valor predeterminado es `kreonyx.local`. Útil para ejecutar múltiples instancias en la misma red.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración KREONYX CLI a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, KREONYX CLI intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración KREONYX CLI a través de la opción `theme`.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/kreonyx/agents/` o `.kreonyx/agents/`. [Más información aquí](/docs/agents).
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, KREONYX CLI recurrirá a `"build"` con una advertencia.

Esta configuración se aplica en todas las interfaces: TUI, CLI (`kreonyx run`), aplicación de escritorio y GitHub Acción.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/kreonyx/commands/` o `.kreonyx/commands/`. [Más información aquí](/docs/commands).
...
KREONYX CLI descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
De forma predeterminada, kreonyx **permite todas las operaciones** sin requerir aprobación explícita. Puede cambiar esto usando la opción `permission`.
...
Los [complementos](/docs/plugins) amplían KREONYX CLI con herramientas, enlaces e integraciones personalizados.

Coloque los archivos de complemento en `.kreonyx/plugins/` o `~/.config/kreonyx/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
...
Esto es útil cuando desea restringir KREONYX CLI para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
