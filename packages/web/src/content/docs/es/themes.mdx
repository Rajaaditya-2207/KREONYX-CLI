...
Con KREONYX CLI puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, KREONYX CLI usa nuestro propio tema `kreonyx`.
...
KREONYX CLI viene con varios temas integrados.
...
- Quiere que KREONYX CLI coincida con la apariencia de su terminal
...
KREONYX CLI admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/kreonyx/themes/*.json` o `$XDG_CONFIG_HOME/kreonyx/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.kreonyx/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.kreonyx/themes/*.json`
