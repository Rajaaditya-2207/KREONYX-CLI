---
title: Servidor
description: Interactuar con el servidor kreonyx a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `kreonyx serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente kreonyx puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `kreonyx.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `kreonyx`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `kreonyx serve` como a `kreonyx web`.
...
Cuando ejecuta `kreonyx`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor kreonyx para interactuar con kreonyx mediante programación.
:::

Esta arquitectura permite que kreonyx admita múltiples clientes y le permite interactuar con kreonyx mediante programación.

Puede ejecutar `kreonyx serve` para iniciar un servidor independiente. Si tienes el
kreonyx TUI ejecutándose, `kreonyx serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos KREONYX CLI [IDE](/docs/ide).
...
El servidor kreonyx expone las siguientes API.
