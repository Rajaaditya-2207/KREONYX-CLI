...
أنشئ الملف `.kreonyx/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات KREONYX CLI أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ KREONYX CLI:
...
- على مستوى النظام: `~/.config/kreonyx/commands/`
- لكل مشروع: `.kreonyx/commands/`
...
يتضمن kreonyx عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
