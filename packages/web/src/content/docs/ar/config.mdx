---
title: الإعدادات
description: استخدام ملف إعدادات KREONYX CLI بصيغة JSON.
---

يمكنك ضبط KREONYX CLI باستخدام ملف إعدادات بصيغة JSON.

---

## التنسيق

يدعم KREONYX CLI تنسيقي **JSON** و**JSONC** (JSON مع تعليقات).
...
على سبيل المثال، إذا كان الإعداد العام يضبط `theme: "kreonyx"` و`autoupdate: true`، وكان إعداد المشروع يضبط `model: "anthropic/claude-sonnet-4-5"`، فستتضمن الإعدادات النهائية الخيارات الثلاثة جميعها.
...
1. **الإعدادات البعيدة** (من `.well-known/kreonyx`) - الإعدادات الافتراضية على مستوى المؤسسة
2. **الإعدادات العامة** (`~/.config/kreonyx/kreonyx.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`OPENCODE_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`kreonyx.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.kreonyx`** - الوكلاء، الأوامر، الإضافات
...
:::note
يستخدم الدليلان `.kreonyx` و`~/.config/kreonyx` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
يمكن للمؤسسات توفير إعدادات افتراضية عبر نقطة النهاية `.well-known/kreonyx`. يتم جلب ذلك تلقائيًا عندما تقوم بالمصادقة مع مزوّد يدعم هذه الميزة.
...
ضع إعدادات KREONYX CLI العامة في `~/.config/kreonyx/kreonyx.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
أضف `kreonyx.json` في جذر مشروعك. تمتلك إعدادات المشروع أعلى أولوية بين ملفات الإعدادات القياسية، إذ تتجاوز الإعدادات العامة والبعيدة معًا.

:::tip
ضع إعدادات المشروع الخاصة في جذر مشروعك.
:::

عند تشغيل KREONYX CLI، يبحث عن ملف إعدادات في الدليل الحالي أو يصعد حتى أقرب دليل Git.
...
الدليل القياسي `.kreonyx`، ويجب أن يتبع نفس البنية.
...
يُحمَّل الدليل المخصص بعد الإعدادات العامة وأدلة `.kreonyx`، لذلك **يمكنه تجاوز** إعداداتها.
...
يمكنك ضبط إعدادات الخادم لأوامر `kreonyx serve` و`kreonyx web` عبر الخيار `server`.
...
- `mdns` - تفعيل اكتشاف الخدمة عبر mDNS. يتيح ذلك للأجهزة الأخرى على الشبكة اكتشاف خادم KREONYX CLI.
- `mdnsDomain` - اسم نطاق مخصص لخدمة mDNS. القيمة الافتراضية هي `kreonyx.local`. مفيد لتشغيل عدة نُسخ على نفس الشبكة.
...
يمكنك ضبط المزوّدين والنماذج التي تريد استخدامها في إعدادات KREONYX CLI عبر الخيارات `provider` و`model` و`small_model`.
...
يضبط الخيار `small_model` نموذجًا منفصلًا للمهام الخفيفة مثل توليد العناوين. افتراضيًا يحاول KREONYX CLI استخدام نموذج أقل تكلفة إذا كان متاحًا لدى مزوّدك، وإلا فسيعود إلى النموذج الرئيسي.
...
يمكنك ضبط السمة التي تريد استخدامها في إعدادات KREONYX CLI عبر الخيار `theme`.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/kreonyx/agents/` أو `.kreonyx/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
يجب أن يكون الوكيل الافتراضي وكيلًا أساسيًا (وليس وكيلًا فرعيًا). يمكن أن يكون وكيلًا مدمجًا مثل `"build"` أو `"plan"`، أو [وكيلًا مخصصًا](/docs/agents) قمت بتعريفه. إذا لم يكن الوكيل المحدد موجودًا أو كان وكيلًا فرعيًا، فسيعود KREONYX CLI إلى `"build"` مع تحذير.

ينطبق هذا الإعداد على جميع الواجهات: TUI وCLI (`kreonyx run`) وتطبيق سطح المكتب وGitHub Action.
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/kreonyx/commands/` أو `.kreonyx/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
سيقوم KREONYX CLI بتنزيل أي تحديثات جديدة تلقائيًا عند بدء التشغيل. يمكنك تعطيل ذلك عبر الخيار `autoupdate`.
...
افتراضيًا يسمح kreonyx **بجميع العمليات** دون الحاجة إلى موافقة صريحة. يمكنك تغيير ذلك عبر الخيار `permission`.
...
[الإضافات](/docs/plugins) توسّع KREONYX CLI بأدوات وخُطافات وتكاملات مخصصة.

ضع ملفات الإضافات في `.kreonyx/plugins/` أو `~/.config/kreonyx/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
...
هذا مفيد عندما تريد تقييد KREONYX CLI لاستخدام مزوّدين محددين بدلًا من تعطيلهم واحدًا تلو الآخر.
