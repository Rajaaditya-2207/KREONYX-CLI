---
title: CLI
description: أوامر وخيارات CLI في KREONYX CLI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ KREONYX CLI افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
kreonyx
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع KREONYX CLI برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ KREONYX CLI.
...
يتضمن CLI الخاص بـ KREONYX CLI أيضا الأوامر التالية.

---

### agent

إدارة وكلاء KREONYX CLI.
...
إرفاق terminal بخادم الواجهة الخلفية لـ KREONYX CLI قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
kreonyx attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ KREONYX CLI تعمل عن بعد. على سبيل المثال:
...
يعتمد KREONYX CLI على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `kreonyx auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/kreonyx/auth.json`.

```bash
kreonyx auth login
```

عند تشغيل KREONYX CLI يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في KREONYX CLI.
...
تشغيل kreonyx في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `kreonyx serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| `--attach`   |         | الإرفاق بخادم kreonyx قيد التشغيل (مثل http://localhost:4096)    |
...
بدء خادم KREONYX CLI بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
kreonyx serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف kreonyx دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `kreonyx`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات KREONYX CLI لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة KREONYX CLI.

```bash
kreonyx import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة KREONYX CLI.
...
بدء خادم KREONYX CLI بدون واجهة مع واجهة ويب.

```bash
kreonyx web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى KREONYX CLI عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `kreonyx`).
...
إلغاء تثبيت KREONYX CLI وإزالة جميع الملفات ذات الصلة.
...
تحديث kreonyx إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر kreonyx الخيارات العامة التالية.
...
يمكن تهيئة KREONYX CLI باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `kreonyx`) |
