---
title: المزوّدون
description: استخدام أي موفّر LLM في KREONYX CLI.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم KREONYX CLI ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات KREONYX CLI.
...
في `~/.local/share/kreonyx/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات KREONYX CLI.
...
KREONYX CLI Zen هي قائمة نماذج يوفّرها فريق KREONYX CLI وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع KREONYX CLI. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع KREONYX CLI Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر kreonyx، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/auth).
...
يعمل مثل أي مزوّد آخر في KREONYX CLI واستخدامه اختياري بالكامل.
...
لاستخدام Amazon Bedrock مع KREONYX CLI:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل kreonyx:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `kreonyx.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل KREONYX CLI غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل kreonyx بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات kreonyx.
...
تخزين الرمز في مساحة تخزين مصادقة kreonyx.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم KREONYX CLI نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر KREONYX CLI
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`kreonyx.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [kreonyx-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### التهيئة

خصّص عبر `kreonyx.json`:
...
لاستخدام اشتراك GitHub Copilot مع kreonyx:
...
لاستخدام Google Vertex AI مع KREONYX CLI:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر kreonyx، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [kreonyx-helicone-session](https://github.com/H2Shami/kreonyx-helicone-session) لتسجيل كل محادثة KREONYX CLI تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة KREONYX CLI مدرجة كجلسة مستقلة.
...
يمكنك ضبط kreonyx لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط kreonyx لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط kreonyx لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع KREONYX CLI. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/kreonyx) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع KREONYX CLI:
...
3. انسخ مفتاح API لاستخدامه في KREONYX CLI.
...
6. **مهم**: قبل استخدام نماذج السحابة في KREONYX CLI، يجب سحب معلومات النموذج محليا:
...
KREONYX CLI Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق KREONYX CLI. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>KREONYX CLI Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **KREONYX CLI Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ KREONYX CLI.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات kreonyx.
...
5. يمكنك أيضا تخصيصها عبر إعدادات kreonyx. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات kreonyx. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع kreonyx. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `kreonyx.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ KREONYX CLI بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `kreonyx auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات kreonyx وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات kreonyx.
