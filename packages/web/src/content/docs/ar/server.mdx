---
title: الخادم
description: تفاعل مع خادم kreonyx عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `kreonyx serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل kreonyx استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `kreonyx.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `kreonyx`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `kreonyx serve` و `kreonyx web`.
...
عند تشغيل `kreonyx` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم kreonyx للتفاعل مع kreonyx برمجيا.
:::

تتيح هذه البنية لـ kreonyx دعم عدة عملاء وتمكّنك من التفاعل مع kreonyx برمجيا.

يمكنك تشغيل `kreonyx serve` لبدء خادم مستقل. إذا كانت واجهة kreonyx في terminal (TUI)
قيد التشغيل، فسيبدأ `kreonyx serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات KREONYX CLI لـ [IDE](/docs/ide).
...
يعرض خادم kreonyx واجهات API التالية.
