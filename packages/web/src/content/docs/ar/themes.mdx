...
مع KREONYX CLI يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم KREONYX CLI سمتنا `kreonyx`.
...
يأتي KREONYX CLI مع عدة سمات مدمجة.
...
- يريدون أن يطابق KREONYX CLI مظهر terminal لديهم
...
يدعم KREONYX CLI نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/kreonyx/themes/*.json` أو `$XDG_CONFIG_HOME/kreonyx/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.kreonyx/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.kreonyx/themes/*.json`
