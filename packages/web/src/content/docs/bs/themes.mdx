...
U KREONYX CLI mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, KREONYX CLI koristi nasu `kreonyx` temu.
...
KREONYX CLI dolazi sa vise ugradenih tema.
...
- Zele da KREONYX CLI odgovara izgledu njihovog terminala
...
KREONYX CLI podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/kreonyx/themes/*.json` ili `$XDG_CONFIG_HOME/kreonyx/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.kreonyx/themes/*.json`
4. **Trenutni radni direktorij** - `./.kreonyx/themes/*.json`
