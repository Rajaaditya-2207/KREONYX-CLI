...
Možete dodati vanjske alate u KREONYX CLI koristeći _Model Context Protocol_, ili MCP. KREONYX CLI podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [KREONYX CLI Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/kreonyx`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
KREONYX CLI automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, KREONYX CLI će:
...
Ako server zahtijeva autentifikaciju, KREONYX CLI će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `kreonyx mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, KREONYX CLI sigurno čuva tokene u `~/.local/share/kreonyx/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u KREONYX CLI, zajedno s ugrađenim alatima. Možete njima upravljati kroz KREONYX CLI konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete KREONYX CLI sa vašim Sentry nalogom.
