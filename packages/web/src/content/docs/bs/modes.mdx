...
ove ili konfigurirajte svoje putem kreonyx config.
...
kreonyx dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.kreonyx/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `kreonyx.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/kreonyx/modes/`
- Projekat: `.kreonyx/modes/`
...
Ako temperatura nije navedena, kreonyx koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.kreonyx/modes/` za specifične načine rada ili `~/.config/kreonyx/modes/` za globalne načine:
