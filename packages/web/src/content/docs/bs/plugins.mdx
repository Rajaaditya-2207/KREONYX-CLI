---
title: Dodaci
description: Napišite vlastite dodatke za proširenje KREONYX CLI.
---

Dodaci vam omogućavaju da proširite KREONYX CLI spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja KREONYX CLI.
...
- `.kreonyx/plugins/` - Dodaci na nivou projekta
- `~/.config/kreonyx/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/kreonyx/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/kreonyx/kreonyx.json`)
2. Konfiguracija projekta (`kreonyx.json`)
3. Globalni direktorij dodataka (`~/.config/kreonyx/plugins/`)
4. Direktorij dodataka projekta (`.kreonyx/plugins/`)
...
KREONYX CLI pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje KREONYX CLI.
...
:::note
Ako koristite desktop aplikaciju KREONYX CLI, ona može automatski slati sistemske obavijesti kada je odgovor spreman ili kada dođe do greške u sesiji.
:::
...
Spriječite kreonyx da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u kreonyx:
...
Pomoćnik `tool` kreira prilagođeni alat koji kreonyx može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za kreonyx zajedno sa ugrađenim alatima.
