...
Agent skills omogucavaju KREONYX CLI da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
KREONYX CLI pretrazuje ove lokacije:

- Konfiguracija projekta: `.kreonyx/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/kreonyx/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, KREONYX CLI ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.kreonyx/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/kreonyx/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.kreonyx/skills/git-release/SKILL.md` ovako:
...
KREONYX CLI navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `kreonyx.json`:
...
**Za ugradene agente** (u `kreonyx.json`):
