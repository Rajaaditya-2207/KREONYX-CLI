---
title: CLI
description: KREONYX CLI CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KREONYX CLI CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
kreonyx
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa KREONYX CLI.
...
Pokrenite KREONYX CLI terminalski korisnički interfejs.
...
KREONYX CLI CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte KREONYX CLI agentima.
...
Priključite terminal na već pokrenut KREONYX CLI backend server pokrenut putem `serve` ili `web` naredbi.

```bash
kreonyx attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim KREONYX CLI backend-om. Na primjer:
...
KREONYX CLI pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `kreonyx auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/kreonyx/auth.json`.

```bash
kreonyx auth login
```

Kada se KREONYX CLI pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u KREONYX CLI.
...
Pokrenite KREONYX CLI u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `kreonyx serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| `--attach`   |        | Priključite na pokrenuti KREONYX CLI server (npr. http://localhost:4096)    |
...
Pokrenite KREONYX CLI headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
kreonyx serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti KREONYX CLI-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `kreonyx`).
...
Upravljajte KREONYX CLI sesijama.
...
Navedite sve KREONYX CLI sesije.
...
Prikaži statistiku upotrebe tokena i troškova za vaše KREONYX CLI sesije.
...
Uvezite podatke sesije iz JSON datoteke ili KREONYX CLI dijeljenog URL-a.

```bash
kreonyx import <file>
```

Možete uvesti iz lokalne datoteke ili KREONYX CLI dijeljenog URL-a.
...
Pokrenite KREONYX CLI headless server sa web interfejsom.

```bash
kreonyx web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup KREONYX CLI-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `kreonyx`).
...
Deinstalirajte KREONYX CLI i uklonite sve povezane datoteke.
...
Ažurira KREONYX CLI na najnoviju verziju ili određenu verziju.
...
KREONYX CLI CLI prihvata sljedeće globalne zastavice.
...
KREONYX CLI se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `kreonyx`) |
