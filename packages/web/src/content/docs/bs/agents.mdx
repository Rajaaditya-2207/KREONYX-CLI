...
Postoje dvije vrste agenata u KREONYX CLI: primarni agenti i podagenti.
...
KREONYX CLI dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
KREONYX CLI dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

KREONYX CLI dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `kreonyx.json`:
...
- Globalno: `~/.config/kreonyx/agents/`
- Po projektu: `.kreonyx/agents/`
...
Ako temperatura nije navedena, KREONYX CLI koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu KREONYX CLI konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj KREONYX CLI konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [KREONYX CLI Zen](/docs/zen), koristili biste `kreonyx/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `kreonyx models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/kreonyx).
:::
