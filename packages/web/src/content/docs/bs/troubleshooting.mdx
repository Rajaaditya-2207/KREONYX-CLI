...
Da biste otklonili probleme s KREONYX CLI, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\kreonyx\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `kreonyx --log-level DEBUG`.

---

## Pohrana

KREONYX CLI pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/kreonyx/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\kreonyx`
...
KREONYX CLI Desktop pokreće lokalni KREONYX CLI server (`kreonyx-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `KREONYX CLI` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/kreonyx/kreonyx.jsonc` (ili `~/.config/kreonyx/kreonyx.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/kreonyx/kreonyx.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\kreonyx\kreonyx.jsonc`
...
KREONYX CLI također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/kreonyx/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\kreonyx\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga KREONYX CLI mogao ponovo izgraditi.

1. Potpuno zatvorite KREONYX CLI Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/kreonyx`
- **Linux**: obrišite `~/.cache/kreonyx` (ili pokrenite `rm -rf ~/.cache/kreonyx`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\kreonyx`

3. Ponovo pokrenite KREONYX CLI Desktop.

---

### Rješavanje problema sa vezom na serveru

KREONYX CLI Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `kreonyx.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, KREONYX CLI Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa KREONYX CLI karakteristikama.

---

### Obavještenja se ne prikazuju

KREONYX CLI Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za KREONYX CLI u postavkama vašeg OS-a, i
...
1. Zatvorite KREONYX CLI Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka KREONYX CLI Desktop aplikacije):

- `kreonyx.settings.dat` (URL zadanog servera za desktop)
- `kreonyx.global.dat` i `kreonyx.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s KREONYX CLI:
...
Da saznate kojim modelima imate pristup, pokrenite `kreonyx models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\kreonyx`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. KREONYX CLI dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\kreonyx`

2. Ponovo pokrenite KREONYX CLI da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti KREONYX CLI da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
KREONYX CLI će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
