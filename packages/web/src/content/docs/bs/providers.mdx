---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u KREONYX CLI.
---

import config from "../../../../config.mjs"
export const console = config.console

KREONYX CLI koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj KREONYX CLI konfiguraciji.
...
u `~/.local/share/kreonyx/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem KREONYX CLI
config.
...
KREONYX CLI Zen je lista modela koje je obezbedio KREONYX CLI tim koji su bili
testirano i potvrđeno da dobro radi sa KREONYX CLI. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa KREONYX CLI Zen.
:::
...
Radi kao i svaki drugi provajder u KREONYX CLI i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s KREONYX CLI:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete kreonyx:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `kreonyx.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u KREONYX CLI.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi kreonyx ispravno radio.
   :::
...
Također možete dodati modele kroz svoju kreonyx konfiguraciju.
...
da pohrani token u kreonyx auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
KREONYX CLI koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate KREONYX CLI
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`kreonyx.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [kreonyx-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `kreonyx.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s kreonyx:
...
Za korištenje Google Vertex AI s KREONYX CLI:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće kreonyx.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem kreonyxa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [kreonyx-helicone-session](https://github.com/H2Shami/kreonyx-helicone-session) da automatski prijavite svaki KREONYX CLI razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki KREONYX CLI razgovor naveden kao zasebna sesija.
...
Možete konfigurirati kreonyx za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati kreonyx za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati kreonyx za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za KREONYX CLI. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/kreonyx) za detalje.
:::
...
Da biste koristili Ollama Cloud s KREONYX CLI:
...
3. Kopirajte API ključ za korištenje u KREONYX CLI.
...
6. **Važno**: Prije upotrebe modela oblaka u KREONYX CLI, morate lokalno povući informacije o modelu:
...
KREONYX CLI Zen je lista testiranih i verifikovanih modela koju je obezbedio KREONYX CLI tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>KREONYX CLI Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **KREONYX CLI Zen**.

```txt
   /connect
```

3. Unesite svoj KREONYX CLI API ključ.
...
Također možete dodati dodatne modele putem vaše kreonyx konfiguracije.
...
5. Također ih možete prilagoditi putem vaše kreonyx konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju kreonyx konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s kreonyx-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `kreonyx.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju KREONYX CLI da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `kreonyx auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite KREONYX CLI konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj kreonyx konfiguraciji.
