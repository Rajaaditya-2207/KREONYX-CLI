---
title: Server
description: Komunicirajte s kreonyx serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `kreonyx serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji kreonyx klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `kreonyx.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `kreonyx`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `kreonyx serve` i za `kreonyx web`.
...
Kada pokrenete `kreonyx`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite kreonyx server za programsku interakciju sa kreonyx-om.
:::

Ova arhitektura omogucava kreonyx podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `kreonyx serve` da startate standalone server. Ako je kreonyx TUI vec pokrenut, `kreonyx serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste KREONYX CLI [IDE](/docs/ide) pluginovi.
...
kreonyx server izlaže sljedece API-je.
