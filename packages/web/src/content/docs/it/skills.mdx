...
Le skill degli agenti permettono a KREONYX CLI di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
KREONYX CLI cerca in queste posizioni:

- Config di progetto: `.kreonyx/skills/<name>/SKILL.md`
- Config globale: `~/.config/kreonyx/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, KREONYX CLI risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.kreonyx/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/kreonyx/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.kreonyx/skills/git-release/SKILL.md` cosi':
...
KREONYX CLI elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `kreonyx.json`:
...
**Per agenti integrati** (in `kreonyx.json`):
