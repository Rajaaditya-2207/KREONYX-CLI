---
title: CLI
description: Opzioni e comandi della CLI di KREONYX CLI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di KREONYX CLI, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
kreonyx
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con KREONYX CLI in modo programmatico.
...
Avvia l'interfaccia testuale di KREONYX CLI (interfaccia utente da terminale).
...
La CLI di KREONYX CLI include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per KREONYX CLI.
...
Collega un terminale a un backend server di KREONYX CLI già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
kreonyx attach [url]
```

Questo consente di usare la TUI con un backend KREONYX CLI remoto. Per esempio:
...
KREONYX CLI si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `kreonyx auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/kreonyx/auth.json`.

```bash
kreonyx auth login
```

Quando KREONYX CLI si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in KREONYX CLI.
...
Esegui kreonyx in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `kreonyx serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| `--attach`   |       | Attach a un server kreonyx in esecuzione (es. http://localhost:4096)    |
...
Avvia un server KREONYX CLI headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
kreonyx serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di kreonyx senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `kreonyx`).
...
Gestisci le sessioni KREONYX CLI.
...
Elenca tutte le sessioni KREONYX CLI.
...
Mostra statistiche di utilizzo token e costo per le sessioni KREONYX CLI.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione KREONYX CLI.

```bash
kreonyx import <file>
```

Puoi importare da un file locale o da un URL di condivisione KREONYX CLI.
...
Avvia un server KREONYX CLI headless con interfaccia web.

```bash
kreonyx web
```

Avvia un server HTTP e apre un browser per accedere a KREONYX CLI tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `kreonyx`).
...
Disinstalla KREONYX CLI e rimuove tutti i file correlati.
...
Aggiorna kreonyx all'ultima versione o a una versione specifica.
...
La CLI di kreonyx accetta i seguenti flag globali.
...
KREONYX CLI può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `kreonyx`)     |
