---
title: Supporto ACP
description: Usa KREONYX CLI in qualunque editor compatibile con ACP.
---

KREONYX CLI supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare KREONYX CLI via ACP, configura l'editor in modo che esegua il comando `kreonyx acp`.

Il comando avvia KREONYX CLI come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'KREONYX CLI' nel selettore agenti di AI Chat.
...
Per usare KREONYX CLI come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare KREONYX CLI come agente ACP per la chat.
...
KREONYX CLI funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione KREONYX CLI
