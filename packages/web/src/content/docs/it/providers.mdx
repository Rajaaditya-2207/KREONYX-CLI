---
title: Provider
description: Usa qualsiasi provider LLM in KREONYX CLI.
---

import config from "../../../../config.mjs"
export const console = config.console

KREONYX CLI usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di KREONYX CLI.
...
KREONYX CLI Zen e una lista di modelli forniti dal team KREONYX CLI, testati e
verificati per funzionare bene con KREONYX CLI. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da KREONYX CLI Zen.
:::

1. Esegui `/connect` nella TUI, seleziona kreonyx e vai su [opencode.ai/auth](https://opencode.ai/auth).
...
Funziona come qualsiasi altro provider in KREONYX CLI ed e totalmente opzionale.
...
Per usare Amazon Bedrock con KREONYX CLI:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   Imposta una di queste variabili d'ambiente mentre esegui kreonyx:
   ...
   Per configurazione specifica del progetto o persistente, usa `kreonyx.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in KREONYX CLI non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché kreonyx funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di kreonyx.
...
salvare il token nello storage di auth di kreonyx.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
KREONYX CLI usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare KREONYX CLI
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `kreonyx.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [kreonyx-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configurazione

Personalizza tramite `kreonyx.json`:
...
Per usare il tuo abbonamento GitHub Copilot con kreonyx:
...
Per usare Google Vertex AI con KREONYX CLI:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui kreonyx.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da kreonyx, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [kreonyx-helicone-session](https://github.com/H2Shami/kreonyx-helicone-session) per loggare automaticamente ogni conversazione di KREONYX CLI come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di KREONYX CLI elencata come sessione separata.
...
Puoi configurare kreonyx per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare kreonyx per usare modelli locali tramite LM Studio.
...
Puoi configurare kreonyx per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per KREONYX CLI. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/kreonyx) per dettagli.
:::
...
Per usare Ollama Cloud con KREONYX CLI:
...
3. Copia la chiave API da usare in KREONYX CLI.
...
6. **Importante**: Prima di usare modelli cloud in KREONYX CLI, devi scaricare le informazioni del modello localmente:
...
KREONYX CLI Zen è una lista di modelli testati e verificati forniti dal team KREONYX CLI. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>KREONYX CLI Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **KREONYX CLI Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di KREONYX CLI.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di kreonyx.
...
5. Puoi anche personalizzarli tramite la configurazione di kreonyx. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di kreonyx. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con kreonyx. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `kreonyx.json` nella directory del progetto:
...
I campi `limit` permettono a KREONYX CLI di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `kreonyx auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di kreonyx e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config kreonyx.
