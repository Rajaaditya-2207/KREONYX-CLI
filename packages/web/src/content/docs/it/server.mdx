---
title: Server
description: Interagisci con il server kreonyx via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `kreonyx serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client kreonyx.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `kreonyx.local` |
...
Imposta `OPENCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `kreonyx`, oppure imposta `OPENCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `kreonyx serve` sia per `kreonyx web`.
...
Quando esegui `kreonyx` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server kreonyx per interagire con kreonyx in modo programmatico.
:::

Questa architettura permette a kreonyx di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `kreonyx serve` per avviare un server standalone. Se la TUI di kreonyx e gia in esecuzione, `kreonyx serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di KREONYX CLI.
...
Il server kreonyx espone le seguenti API.
