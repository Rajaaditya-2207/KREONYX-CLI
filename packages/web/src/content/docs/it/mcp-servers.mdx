...
Puoi aggiungere strumenti esterni a KREONYX CLI usando il _Model Context Protocol_, o MCP. KREONYX CLI supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [KREONYX CLI Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/kreonyx`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
KREONYX CLI gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, KREONYX CLI:
...
Se il server richiede autenticazione, KREONYX CLI ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `kreonyx mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, KREONYX CLI memorizzera' i token in modo sicuro in `~/.local/share/kreonyx/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in KREONYX CLI insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione KREONYX CLI come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare KREONYX CLI al tuo account Sentry.
