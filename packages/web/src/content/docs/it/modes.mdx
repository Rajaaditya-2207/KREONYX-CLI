...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di kreonyx. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in kreonyx ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di kreonyx.
...
kreonyx include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.kreonyx/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `kreonyx.json`:
...
- Globale: `~/.config/kreonyx/modes/`
- Progetto: `.kreonyx/modes/`
...
Se non viene specificata alcuna temperatura, kreonyx usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di kreonyx sia per quella specifica del progetto.
...
Crea file di modalita in `.kreonyx/modes/` per modalita specifiche del progetto oppure in `~/.config/kreonyx/modes/` per modalita globali:
