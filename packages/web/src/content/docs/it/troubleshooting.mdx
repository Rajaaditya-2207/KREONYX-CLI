...
Per diagnosticare problemi con KREONYX CLI, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\kreonyx\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `kreonyx --log-level DEBUG`.

---

## Archiviazione

kreonyx salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/kreonyx/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\kreonyx`
...
KREONYX CLI Desktop esegue in background un server locale di KREONYX CLI (il sidecar `kreonyx-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `KREONYX CLI` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux**: `~/.config/kreonyx/kreonyx.jsonc` (or `~/.config/kreonyx/kreonyx.json`)
- **macOS/Linux** (older installs): `~/.local/share/kreonyx/kreonyx.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\kreonyx\kreonyx.jsonc`
...
KREONYX CLI puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/kreonyx/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\kreonyx\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che KREONYX CLI possa ricostruirla.

1. Quit KREONYX CLI Desktop completely.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/kreonyx`
- **Linux**: elimina `~/.cache/kreonyx` (oppure esegui `rm -rf ~/.cache/kreonyx`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\kreonyx`

3. Restart KREONYX CLI Desktop.

---

### Risolvi problemi di connessione al server

KREONYX CLI Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `kreonyx.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.
...
Su Windows, KREONYX CLI Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di KREONYX CLI.

---

### Notifiche non visualizzate

KREONYX CLI Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per KREONYX CLI nelle impostazioni del sistema operativo, e
...
1. Quit KREONYX CLI Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app KREONYX CLI Desktop):

- `kreonyx.settings.dat` (desktop default server URL)
- `kreonyx.global.dat` e `kreonyx.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con KREONYX CLI:
...
Per capire a quali modelli hai accesso, esegui `kreonyx models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\kreonyx`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. kreonyx installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\kreonyx`

2. Riavvia kreonyx per reinstallare i pacchetti provider piu recenti

Questo forzera kreonyx a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
kreonyx rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
