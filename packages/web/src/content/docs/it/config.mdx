...
description: Usa la configurazione JSON di KREONYX CLI.
---

Puoi configurare KREONYX CLI usando un file di configurazione JSON.

---

## Formato

KREONYX CLI supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
Per esempio, se la tua configurazione globale imposta `theme: "kreonyx"` e `autoupdate: true`, e la configurazione del progetto imposta `model: "anthropic/claude-sonnet-4-5"`, la configurazione finale includera tutte e tre le impostazioni.
...
1. **Config remota** (da `.well-known/kreonyx`) - default dell'organizzazione
2. **Config globale** (`~/.config/kreonyx/kreonyx.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `OPENCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`kreonyx.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.kreonyx`** - agenti, comandi, plugin
...
:::note
Le directory `.kreonyx` e `~/.config/kreonyx` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/kreonyx`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di KREONYX CLI in `~/.config/kreonyx/kreonyx.json`. Usa la configurazione globale per preferenze valide per l'utente (ad es. temi, provider o keybind).
...
Aggiungi `kreonyx.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.

:::tip
Metti la configurazione specifica del progetto nella root del progetto.
:::

Quando KREONYX CLI si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `OPENCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.kreonyx` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.kreonyx`, quindi **puo sovrascrivere** le loro impostazioni.
...
Puoi configurare le impostazioni del server per i comandi `kreonyx serve` e `kreonyx web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server KREONYX CLI.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `kreonyx.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in KREONYX CLI tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, KREONYX CLI prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi configurare il tema da usare in KREONYX CLI tramite l'opzione `theme`.
...
Puoi anche definire agenti usando file markdown in `~/.config/kreonyx/agents/` o `.kreonyx/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, KREONYX CLI fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`kreonyx run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/kreonyx/commands/` o `.kreonyx/commands/`. [Scopri di piu](/docs/commands).
...
KREONYX CLI scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, kreonyx **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono KREONYX CLI con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.kreonyx/plugins/` o `~/.config/kreonyx/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare KREONYX CLI a usare solo alcuni provider invece di disabilitarli uno per uno.
