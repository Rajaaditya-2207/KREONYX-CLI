---
title: Plugin
description: Scrivi plugin per estendere KREONYX CLI.
---

I plugin ti permettono di estendere KREONYX CLI agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di KREONYX CLI.
...
- `.kreonyx/plugins/` - plugin a livello progetto
- `~/.config/kreonyx/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/kreonyx/node_modules/`.
...
1. Config globale (`~/.config/kreonyx/kreonyx.json`)
2. Config di progetto (`kreonyx.json`)
3. Directory plugin globale (`~/.config/kreonyx/plugins/`)
4. Directory plugin di progetto (`.kreonyx/plugins/`)
...
KREONYX CLI esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di kreonyx per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere kreonyx.
...
:::note
Se usi l'app desktop di KREONYX CLI, puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
Impedisci a kreonyx di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a kreonyx:
...
L'helper `tool` crea uno strumento personalizzato che kreonyx puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in kreonyx insieme agli strumenti integrati.
