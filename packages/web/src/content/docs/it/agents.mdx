...
In KREONYX CLI esistono due tipi di agenti: agenti primari e subagenti.
...
KREONYX CLI include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
KREONYX CLI include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

KREONYX CLI include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `kreonyx.json`:
...
- Globale: `~/.config/kreonyx/agents/`
- Per progetto: `.kreonyx/agents/`
...
Se non specifichi una temperature, KREONYX CLI usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di KREONYX CLI sia per la config specifica del progetto.
...
L'ID modello nella configurazione KREONYX CLI usa il formato `provider/model-id`. Per esempio, se stai usando [KREONYX CLI Zen](/docs/zen), useresti `kreonyx/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `kreonyx models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/kreonyx).
:::
