---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di KREONYX CLI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

KREONYX CLI fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire KREONYX CLI avvia la TUI per la directory corrente.
...
Quando usi la TUI di KREONYX CLI, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a KREONYX CLI. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da KREONYX CLI. _Alias_: `/quit`, `/q`
...
Puoi personalizzare il comportamento della TUI tramite il file di config di KREONYX CLI.
