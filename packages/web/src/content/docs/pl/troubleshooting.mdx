...
Aby debugować problemy z kreonyx, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\kreonyx\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `kreonyx --log-level DEBUG`.

---

## Przechowywanie danych

kreonyx przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/kreonyx/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\kreonyx`
...
kreonyx Desktop uruchamia w tle lokalny serwer kreonyx (sidecar `kreonyx-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `KREONYX CLI` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/kreonyx/kreonyx.jsonc` (lub `~/.config/kreonyx/kreonyx.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/kreonyx/kreonyx.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\kreonyx\kreonyx.jsonc`
...
kreonyx może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/kreonyx/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\kreonyx\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby kreonyx mógł ją odbudować.

1. Całkowicie zamknij kreonyx Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/kreonyx`
- **Linux**: usuń `~/.cache/kreonyx` (lub uruchom `rm -rf ~/.cache/kreonyx`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\kreonyx`

3. Uruchom ponownie kreonyx Desktop.

---

### Napraw problemy z połączeniem z serwerem

kreonyx Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `kreonyx.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows kreonyx Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami kreonyx.

---

### Brak powiadomień

kreonyx Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla kreonyx w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie kreonyx Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji kreonyx Desktop):

- `kreonyx.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `kreonyx.global.dat` i `kreonyx.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `kreonyx upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `kreonyx models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\kreonyx`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. kreonyx dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\kreonyx`

2. Uruchom ponownie kod kreonyx, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to kreonyx do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
kreonyx wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
