---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w kreonyx.
---

import config from "../../../../config.mjs"
export const console = config.console

kreonyx używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji kreonyx.
...
w `~/.local/share/kreonyx/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim kreonyx
config.
...
KREONYX CLI Zen to lista modeli dostarczonych przez zespół kreonyx, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z kreonyx. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od KREONYX CLI Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz kreonyx i przejdź do [opencode.ai/auth](https://opencode.ai/auth).
...
Działa jak każdy inny dostawca w kreonyx i jest całkowicie opcjonalny w użyciu.
...
Aby używać Amazon Bedrock z kreonyx:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania kreonyx:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `kreonyx.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w kreonyx nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod kreonyx działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji kreonyx.
...
kreonyx używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować kreonyx
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`kreonyx.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [kreonyx-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Konfiguracja

Customize through `kreonyx.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem kreonyx:
...
Aby używać Google Vertex AI z kreonyx:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Set them while running kreonyx.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą kreonyx, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [kreonyx-helicone-session](https://github.com/H2Shami/kreonyx-helicone-session), aby automatycznie rejestrować każdą konwersację kreonyx jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja kreonyx będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować kreonyx tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować kreonyx tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować kreonyx tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem kreonyx. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/kreonyx).
:::
...
Aby korzystać z Ollama Cloud z kreonyx:
...
3. Skopiuj klucz API do użycia w kreonyx.
...
6. **Ważne**: Przed użyciem modeli chmurowych w kreonyx musisz pobrać informacje o modelu lokalnie:
...
KREONYX CLI Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół kreonyx. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>KREONYX CLI Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **KREONYX CLI Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API kreonyx.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji kreonyx.
...
5. Możesz je także dostosować za pomocą konfiguracji kreonyx. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z kreonyx:
...
Możesz także dostosować modele za pomocą konfiguracji kreonyx. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z kreonyx. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `kreonyx.json` w katalogu projektu:
...
Pola `limit` pozwalają kreonyx zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `kreonyx auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację kreonyx i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji kreonyx.
