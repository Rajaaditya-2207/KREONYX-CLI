...
Umiejętności agenta pozwalają kreonyx odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
kreonyx przeszukuje te lokalizacje:

- Project config: `.kreonyx/skills/<name>/SKILL.md`
- Global config: `~/.config/kreonyx/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu kreonyx przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.kreonyx/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/kreonyx/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.kreonyx/skills/git-release/SKILL.md` w ten sposób:
...
kreonyx wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `kreonyx.json`:
...
**Dla agentów wbudowanych** (w `kreonyx.json`):
