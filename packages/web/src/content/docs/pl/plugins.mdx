---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć kreonyx.
---

Wtyczki umożliwiające wyłączenie kreonyx poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania kreonyx.
...
- `.kreonyx/plugins/` - Wtyczki na poziomie projektu
- `~/.config/kreonyx/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/kreonyx/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/kreonyx/kreonyx.json`)
2. Konfiguracja projektu (`kreonyx.json`)
3. Globalny katalog wtyczek (`~/.config/kreonyx/plugins/`)
4. Katalog wtyczek projektu (`.kreonyx/plugins/`)
...
kreonyx uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z kreonyx do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do kreonyx.
...
:::note
Jeśli korzystasz z aplikacji komputerowej kreonyx, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj kreonyx czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do kreonyx:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod kreonyx. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
