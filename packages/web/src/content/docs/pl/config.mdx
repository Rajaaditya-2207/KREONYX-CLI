---
title: Konfiguracja
description: Ustawienia KREONYX CLI w pliku JSON.
---

Możesz dostosować KREONYX CLI za pomocą pliku konfiguracyjnego JSON.

---

## Format

KREONYX CLI obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "kreonyx"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/kreonyx`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/kreonyx/kreonyx.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `OPENCODE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`kreonyx.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.kreonyx`** - agenci, polecenia, umiejętności
...
:::note
W katalogach `.kreonyx` i `~/.config/kreonyx` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/kreonyx`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację KREONYX CLI w `~/.config/kreonyx/kreonyx.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak motywy, domyślny dostawca lub skróty klawiszowe.
...
Dodaj `kreonyx.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.

:::tip
Dodaj ten plik do kontroli wersji, aby udostępniać konfigurację zespołowi.
:::

Kiedy KREONYX CLI się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `OPENCODE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.kreonyx` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.kreonyx`, więc **może być łączony** z ich zawartością.
...
Możesz skonfigurować serwer dla `kreonyx serve` i `kreonyx web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer KREONYX CLI.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `kreonyx.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie KREONYX CLI wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Skonfiguruj motyw interfejsu KREONYX CLI za pomocą opcji `theme`.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/kreonyx/agents/` lub `.kreonyx/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, KREONYX CLI powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`kreonyx run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/kreonyx/commands/` lub `.kreonyx/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
KREONYX CLI może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy KREONYX CLI nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie KREONYX CLI **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają KREONYX CLI o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.kreonyx/plugins/` lub `~/.config/kreonyx/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie KREONYX CLI tylko do określonych dostawców.
