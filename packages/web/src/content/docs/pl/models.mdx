...
kreonyx używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługi uruchamiania modeli pierwszych.

---

## Dostawcy

Dostępna usługa dostawcy jest dostępna. Jeśli otrzymasz poświadczenie dostawcy za pomocą polecenia `/connect`, będzie on dostępny po uruchomieniu kreonyx.
...
Oto kilka modeli, które dobrze współpracują z kreonyx, w kolejności. (Nie jest to lista wyczerpująca i nie jest konieczna aktualna):
...
Tutaj pełny identyfikator do `provider_id/model_id`. Na przykład, użycie [KREONYX CLI Zen](/docs/zen), przestrzeganie `kreonyx/gpt-5.1-codex` dla Kodeksu GPT 5.1.
...
Wiele modeli obsługuje wiele wariantów lub różnych wybranych. kreonyx jest dostarczany z podstawowymi wariantami wariantów dla dostawców.

### Wbudowane warianty

kreonyx jest dostarczany z domyślnymi wariantami dla wielu dostawców:
...
Po uruchomieniu kreonyx sprawdzanie modeli w następującej kolejności:

1. Flaga wiersza autora `--model` lub `-m`. Format jest taki sam jak w pliku konfiguracyjnym: `provider_id/model_id`.

2. Lista modeli w konstrukcji kreonyx.
