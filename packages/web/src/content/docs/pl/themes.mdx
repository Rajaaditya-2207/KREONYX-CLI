...
Dzięki kreonyx możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie kreonyx używa naszego własnego motywu `kreonyx`.
...
kreonyx ma kilka wbudowanych motywów.
...
- Want kreonyx to match their terminal's appearance
...
kreonyx obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/kreonyx/themes/*.json` lub `$XDG_CONFIG_HOME/kreonyx/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.kreonyx/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.kreonyx/themes/*.json`
