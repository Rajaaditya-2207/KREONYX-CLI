...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji kreonyx.
...
te lub skonfiguruj własne za pomocą konfiguracji kreonyx.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.kreonyx/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `kreonyx.json`:
...
- Globalnie: `~/.config/kreonyx/modes/`
...
Jeśli nie ma wpływu na temperaturę, kreonyx stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna kreonyx, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.kreonyx/modes/` dla trybów zapisanych dla projektu lub `~/.config/kreonyx/modes/` dla trybów globalnych:
