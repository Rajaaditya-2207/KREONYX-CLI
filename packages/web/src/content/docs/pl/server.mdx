---
title: Serwer
description: Interakcja z serwerem kreonyx za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `kreonyx serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient kreonyx.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `kreonyx.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `kreonyx` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `kreonyx serve`, jak i `kreonyx web`.
...
Kiedy uruchomisz `kreonyx`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera kreonyx do programowej interakcji z kreonyx.
:::

Ta architektura umożliwia obsługę wielu klientów przez kreonyx i programową interakcję z kodem otwartym.

Możesz uruchomić `kreonyx serve`, aby uruchomić samodzielny serwer. Jeśli masz
kreonyx TUI działa, `kreonyx serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki kreonyx [IDE](/docs/ide).
...
Serwer kreonyx udostępnia następujące interfejsy API.
