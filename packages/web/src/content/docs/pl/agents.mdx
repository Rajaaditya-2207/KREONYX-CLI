...
W KREONYX CLI obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
KREONYX CLI zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
KREONYX CLI ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

KREONYX CLI ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `kreonyx.json`:
...
- Globalnie: `~/.config/kreonyx/agents/`
- W projekcie: `.kreonyx/agents/`
...
Jeśli nie zostanie skonfigurowana, KREONYX CLI używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji KREONYX CLI, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji KREONYX CLI w `provider/model-id`. Na przykład, [KREONYX CLI Zen](/docs/zen), użyj `kreonyx/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `kreonyx models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/kreonyx).
:::
