...
Narzędzia stosowane do funkcji, z których LLM może korzystać podczas rozmów. Współpracują z [wbudowanymi narzędziami] (./tools) kreonyx, wtyczka jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.kreonyx/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/kreonyx/tools/`.
