---
title: CLI
description: Opcje i polecenia wiersza poleceń KREONYX CLI.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI KREONYX CLI domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
kreonyx
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać KREONYX CLI programowo.
...
Uruchamia interfejs terminalowy (TUI) KREONYX CLI.
...
Interfejs CLI KREONYX CLI zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami KREONYX CLI.
...
Dołącz terminal do już działającego serwera backendu KREONYX CLI uruchomionego za pomocą `serve` lub `web`.

```bash
kreonyx attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem KREONYX CLI. Na przykład:
...
KREONYX CLI integruje się z wieloma dostawcami. Możesz użyć `kreonyx auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/kreonyx/auth.json`.

```bash
kreonyx auth login
```

Podczas uruchamiania KREONYX CLI ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w KREONYX CLI.
...
Uruchom KREONYX CLI w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `kreonyx serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| `--attach`   |       | Dołącz do działającego serwera KREONYX CLI (np. http://localhost:4096)     |
...
Uruchom serwer KREONYX CLI (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
kreonyx serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API KREONYX CLI bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `kreonyx`).
...
Lista wszystkich sesji KREONYX CLI.
...
Pokaż statystyki wykorzystania tokenów i sesji KREONYX CLI.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania KREONYX CLI.
...
Uruchom serwer KREONYX CLI z interfejsem internetowym.

```bash
kreonyx web
```

Uruchamia to serwer HTTP i udostępnia KREONYX CLI przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `kreonyx`).
...
Odinstaluj KREONYX CLI i usuń wszystkie powiązane pliki.
...
Aktualizuj KREONYX CLI do najnowszej lub określonej wersji.
...
Interfejs CLI KREONYX CLI przyjmuje następujące flagi globalne dla każdego polecenia.
...
KREONYX CLI można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `kreonyx`)     |
