...
kreonyx'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
kreonyx, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
kreonyx, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

kreonyx iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `kreonyx.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse kreonyx modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global kreonyx yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
kreonyx hesabınızdaki model kimliğini `provider/model-id` biçimini kullanır. Örneğin, [KREONYX CLI Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `kreonyx/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `kreonyx models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/kreonyx).
:::
