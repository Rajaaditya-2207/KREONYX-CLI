---
title: Eklentiler
description: kreonyx'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek kreonyx'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya kreonyx'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.kreonyx/plugins/` - Proje düzeyinde eklentiler
- `~/.config/kreonyx/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/kreonyx/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/kreonyx/kreonyx.json`)
2. Project config (`kreonyx.json`)
3. Global eklenti dizini (`~/.config/kreonyx/plugins/`)
4. Proje eklenti dizini (`.kreonyx/plugins/`)
...
kreonyx bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşime geçmek için kreonyx'lu bir SDK istemcisi.
...
kreonyx'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
kreonyx masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
kreonyx'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca kreonyx'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, kreonyx'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:
