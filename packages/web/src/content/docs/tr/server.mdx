---
title: Sunucu
description: kreonyx sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`kreonyx serve` komutu, kreonyx istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `kreonyx.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OPENCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `kreonyx` değeridir; değiştirmek isterseniz `OPENCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `kreonyx serve` hem de `kreonyx web` için geçerlidir.
...
`kreonyx` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
kreonyx ile programatik etkileşim için kreonyx sunucusunu kullanın.
:::

Bu mimari, kreonyx'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`kreonyx serve` ile bağımsız bir sunucu başlatabilirsiniz. kreonyx TUI açıksa, `kreonyx serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum kreonyx [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
kreonyx sunucusu aşağıdaki API'leri sunar.
