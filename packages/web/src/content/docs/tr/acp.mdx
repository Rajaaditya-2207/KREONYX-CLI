---
title: ACP Desteği
description: kreonyx'u ACP uyumlu herhangi bir editörde kullanın.
---

kreonyx, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
kreonyx'u ACP aracılığıyla kullanmak için düzenleyicinizi `kreonyx acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, kreonyx'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'KREONYX CLI' ajanını kullanın.
...
kreonyx'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı kreonyx'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
kreonyx, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- kreonyx yapılandırmanızda yapılandırılmış MCP sunucuları
