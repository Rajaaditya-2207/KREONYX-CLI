...
kreonyx ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak kreonyx kendi `kreonyx` temasını kullanır.
...
kreonyx birden fazla yerleşik temayla gelir.
...
- kreonyx'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
kreonyx, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/kreonyx/themes/*.json` veya `$XDG_CONFIG_HOME/kreonyx/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.kreonyx/themes/*.json`
4. **Geçerli çalışma dizini** - `./.kreonyx/themes/*.json`
