...
kreonyx ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\kreonyx\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `kreonyx --log-level DEBUG`.

---

## Depolama

kreonyx, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/kreonyx/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\kreonyx` yapistirin
...
kreonyx Desktop arka planda yerel bir kreonyx sunucusu (`kreonyx-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `kreonyx` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/kreonyx/kreonyx.jsonc` (veya `~/.config/kreonyx/kreonyx.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/kreonyx/kreonyx.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\kreonyx\kreonyx.jsonc` yapistirin
...
kreonyx diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/kreonyx/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\kreonyx\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), kreonyx'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. kreonyx Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/kreonyx`
- **Linux**: `~/.cache/kreonyx` dizinini silin (veya `rm -rf ~/.cache/kreonyx` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\kreonyx` yapistirin

3. kreonyx Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

kreonyx Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`kreonyx.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta kreonyx Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, kreonyx ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

kreonyx Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda kreonyx icin bildirimler etkinse
...
1. kreonyx Desktop'u kapatin
2. Su dosyalari bulun ve silin (kreonyx Desktop uygulama veri dizininde yer alirlar):

- `kreonyx.settings.dat` (desktop varsayilan sunucu URL'si)
- `kreonyx.global.dat` ve `kreonyx.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
kreonyx ile ilgili bir sorun yasiyorsaniz:
...
3. `kreonyx upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `kreonyx models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\kreonyx`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. kreonyx, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\kreonyx`

2. En guncel provider paketlerini yeniden kurmak icin kreonyx'u yeniden baslatin

Bu, kreonyx'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
kreonyx, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
