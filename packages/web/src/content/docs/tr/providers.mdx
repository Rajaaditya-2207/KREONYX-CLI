---
title: Sağlayıcılar
description: kreonyx'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

kreonyx, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı kreonyx yapılandırmanızda yapılandırın.
...
`~/.local/share/kreonyx/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları kreonyx'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
KREONYX CLI Zen, kreonyx ekibi tarafından sağlanan modellerin bir listesidir.
kreonyx ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz KREONYX CLI Zen ile başlamanızı öneririz.
:::
...
kreonyx'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.
...
Amazon Bedrock'u kreonyx ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   kreonyx'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `kreonyx.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi kreonyx'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   kreonyx'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in kreonyx auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
kreonyx, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. kreonyx'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`kreonyx.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [kreonyx-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`kreonyx.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi kreonyx'la kullanmak için:
...
Google Vertex AI'yi kreonyx ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları kreonyx çalıştırılırken ayarlayın.
...
Helicone'un kreonyx aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her kreonyx görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [kreonyx-helicone-session](https://github.com/H2Shami/kreonyx-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her kreonyx görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
kreonyx'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla kreonyx'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla kreonyx'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini kreonyx için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/kreonyx)'a bakın.
:::
...
Ollama Cloud'u kreonyx ile kullanmak için:
...
3. kreonyx'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: kreonyx'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
KREONYX CLI Zen, kreonyx ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>KREONYX CLI Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **KREONYX CLI Zen**'i arayın.

   ```txt
   /connect
   ```

3. kreonyx API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   kreonyx yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca kreonyx yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri kreonyx yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı kreonyx ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `kreonyx.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları kreonyx'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `kreonyx auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için kreonyx yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, kreonyx yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
