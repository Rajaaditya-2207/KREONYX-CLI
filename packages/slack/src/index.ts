...
import { createOpencode, type ToolPart } from "@kreonyx/sdk"
...
console.log("🚀 Starting kreonyx server...")
const kreonyx = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await kreonyx.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new kreonyx session...")
    const { client, server } = kreonyx
    ...
    console.log("✅ Created kreonyx session:", createResult.data.id)
  ...
  console.log("📝 Sending to kreonyx:", message.text)
